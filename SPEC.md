...
1. **Story feeds**
   ...
   - **The feed is a frozen "materialized set" — nothing reorders or vanishes under the reader between materializations.** A live feed reflows on every store change (a pin from another device yanks a row to the top, a dismiss from another device makes a row disappear, a background refetch injects new articles mid-scroll). To keep the reading surface stable, each feed view renders from a *frozen snapshot* — an ordered top-pin block plus a body, computed at a materialization moment and held between them (`src/lib/feedSnapshot.ts`). The snapshot is per-session (module memory, keyed by feed): it survives the feed's own remounts — navigating into a story and back — but not a cold app launch. Row *content* (score, comment counts) still updates live from cache; only membership and order freeze.
     - **Materialized set contents.** On a full materialize, every pinned story is lifted into the top block (oldest-pin-first, one unified list, no header, no duplication), and the body is the current feed ranking minus pins, hidden, and dismissed rows. A pin surfaces at the top regardless of where HN ranks it — dropped off the front page, or still ranked on a later not-yet-loaded page. The `/tuning` Preview opts out of the whole snapshot layer (it renders the live filtered set so a tuning experiment always reflects the current rule output).
     - **Between materializations, overlays are drawn in place — the set never reflows:**
       - **Your own pin** (tapping a body row's pin) shows the badge in place; the row keeps its position and only consolidates to the top on the next full materialize.
       - **A server pin** (a pin synced from another device) is treated identically — the badge appears in place on a visible row, no reorder. An off-screen server pin simply isn't shown until the next materialize.
       - **A server dismiss** (Done synced from another device) grays the row *in place* — the title reads struck-through and muted, but the row stays put and fully tappable (still a live link, pin/Done still work). It is not removed until the next compact/materialize.
       - **Your own dismiss** from a feed row (the wide-viewport row Done button) is the one local mutation that removes a row immediately — it collapses out of the set at once (your action, your immediate feedback), never dimmed.
       - **Sweep** hides its unpinned neighbors around a body pin but never reflows the set — nothing snaps to the top; pins consolidate only on a full materialize.
     - **The set re-derives only at these moments:**
       - **Full materialize** — a return after **≥6h** since the last full materialize (the `MATERIALIZE_MAX_AGE_MS` clock), or a **pull-to-refresh**. Pins consolidate to the top, dismissed rows drop out, and new articles come in. A cold launch (no snapshot) also full-materializes, and re-materializes once the stale-gated open refresh settles so it lands the current ranking rather than a stale persisted cache. PTR re-materializes even on byte-identical data — it's keyed off the items query's fetch timestamp (`dataUpdatedAt`), not the item-array reference (React Query's structural sharing keeps that reference stable across a no-op refetch).
       - **Compact** — *any* navigation return / remount (article → back) within the 6h window. Drops the rows the reader has finished with (Done / Hidden) and collapses the gaps in place; it does **not** reorder pins, pull server pins to the top, or bring in new articles. This runs on a fresh mount before paint, so there's no scroll jump.
       - **More** — appends the next page's qualifying rows to the tail of the body, below everything already placed. It **never** touches the top block, compacts, or reorders — deliberately, so nothing shifts under a reader who's scrolled down to load more.
     - Unpinning follows the inverse of pinning: a badge simply disappears in place; the row reflows only on the next full materialize. Item data for a pin already in the loaded window is reused (no refetch/flicker); only pins outside the window cost a fetch — at most one extra `/api/items` batch per feed load, often zero. Rides the existing items proxy; no new infra. Degrades silently if the fetch fails — the main feed still renders.
   - **Feed refresh status (opening after a while).** The persisted React Query cache paints the last-seen list instantly on open, then the feed refetches in the background **when the cached list has gone stale** — every refetch trigger (mount, tab refocus, reconnect) is gated on the app-wide 5-minute `staleTime` (the cache TTL), so opening or refocusing a feed whose snapshot is older than that picks up new stories, while navigating back to a list you were reading seconds ago stays quiet instead of re-checking. (Pull-to-refresh and **More** bypass the TTL to force a check.) The mount trigger deliberately uses React Query's stale-gated `refetchOnMount: true`, *not* `'always'`: the earlier `'always'` ignored the TTL and re-checked on every remount — since opening a story unmounts the feed, that flashed the refresh strip on every "back", far more often than readers expected. A thin status strip at the foot of the list — just above the More / Back-to-top row, so the load-related affordances sit together — reports that background refresh so a stale snapshot is never shown silently: a quiet *"Checking for new stories…"* (with spinner) while the refresh is in flight, and a *"Couldn't load new stories."* row with a **Retry** button when it failed. The strip only appears while rows are already on screen — the first-ever load still owns the loading skeletons / error / empty states. The id-list query (`['storyIds', …]`, the one read that hits Firebase directly rather than the `/api/items` proxy) retries a few times with exponential backoff + jitter (configured via `setQueryDefaults` in `main.tsx`; only statusless network blips retry — never a response that carried an HTTP status, see `feedQueryRetry`) before the failed state shows, so a momentary blip on open self-heals; the "More" page fetch keeps its deliberate bail-on-failure behavior and is not retried. The detection of "showing stale rows because the background refresh failed" relies on the fact that React Query keeps a query's status at `success` (so `isError` stays false) when it already has data and only a refetch fails — see `deriveRefreshState` in `src/hooks/useStoryList.ts`. The `/tuning` Preview opts out of the strip. Cost / reliability: no new network beyond the refetch that already happened; the retry is bounded and scoped to the tiny id-list JSON, so no extra item-proxy or summary spend. **Interaction with the frozen materialized set (above):** a background mount/focus/reconnect refetch keeps row *content* fresh but does **not** inject new articles or reorder — new stories surface only at the next full materialize (a ≥6h return or a pull-to-refresh), never silently mid-session. The strip reports the background content refresh; the reader gets new *stories* by pulling to refresh.
