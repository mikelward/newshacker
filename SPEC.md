...
**Ambiguous UAs are deliberately excluded.** `whatsapp`, `mastodon`, `pinterest`, `quora`, `tumblr` would also catch their respective in-app browsers (not just the link-preview crawlers), and a real user trapped in the rewrite would loop between `/api/og`'s `meta refresh` and the rewrite. The regex is restricted to tokens that uniquely identify crawlers and never appear in normal browsers; we'd rather miss a few crawler previews than break in-app browsing.
