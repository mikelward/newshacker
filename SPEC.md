...
- Meta row sits directly **below** the body: author link, then plain text " · age · N replies" (reply count omitted when there are none), all on one baseline at 13px. The meta row hugs the body above it (no `margin-top`; the toggle button's own 4px top padding is the only gap) so it reads as belonging to the comment it follows, not the one below. The comment's vertical padding is asymmetric on purpose — `8px` top / `4px` bottom — because the meta row's author and toggle buttons each carry an internal 4px vertical padding for tap-target height; a 4px card `padding-bottom` plus that 4px button padding visually matches the 8px gap above the body text, and a symmetric 10/10 used to read as noticeably heavier on the bottom. When the comment is expanded, the children list adds a matching 4px `margin-top` so the gap before the first nested reply's border matches the gap between sibling comments (the parent's own `padding-bottom` only takes effect after all children).
