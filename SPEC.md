...
Row order, left-to-right: **Read article** (hidden on self-posts) → **Upvote** (hidden when logged out) → **Favorite** → **Done** → **More actions ⋮**. Each icon button is 48×48px with ≥8px spacing. The Upvote button uses HN's triangle shape (solid `▲`), colored `--hn-meta` by default and `--hn-orange` when voted; tapping flips local state optimistically and POSTs `/api/vote` in the background, rolling back and toasting on failure. See item 7 under *Features* for the full round-trip.

The **Favorite** button (heart, outline → filled) is the "keepsake" half of the save-this pair; Pin is the "queue" half, and lives in the overflow menu (see below). The **Done** button (Material Symbols `done`, outline → filled on toggle) sits immediately right of Favorite and marks the thread complete: the story is filtered out of every feed (see *Pinned vs. Favorite vs. Done* above) and added to the synced Done list. Done is the most salient per-story action on the comments view — by the time you're reading comments you've usually engaged with the thread, and Done is the common exit. Pinning, by contrast, happens before reading (to save for later) and is more natural from the feed row; on the thread page it's secondary, which is why it lives in the overflow menu. Tapping Done on a pinned story also unpins it; Done and Pin are mutually exclusive. Tapping Done does **not** navigate away; the filled icon is the confirmation, and the user can navigate back when ready. Auto-navigate-on-done is tracked in `TODO.md § Thread action bar` as a possible future change. There is no Done toast — the button state is the single source of truth, matching Favorite.
