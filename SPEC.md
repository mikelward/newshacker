...
3. **Hide unpinned** (right, icon only). A second entry point for the sweep action; shares the broom glyph, tooltip (`"Hide unpinned"` / `"Nothing to hide"` when there's nothing to sweep), `aria-label`, and disabled-when-nothing-to-hide state with the list toolbar's Sweep button above. It's the *same* sweep — clicking it hides the currently fully-visible unpinned rows. Pinned to the right edge of the bar, matching the Sweep button's position in the list toolbar. Testid `sweep-btn-bottom` (the list toolbar's Sweep button keeps the unsuffixed `sweep-btn`). Feed-only — library pages never render this slot.
