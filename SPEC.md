...
- **Combined fetch-failure + browser-event detection** (`src/lib/networkStatus.ts`). `navigator.onLine` on mobile lags badly behind reality — walking into a tunnel can leave it stuck at `true` for tens of seconds, so the header pill would appear long after Brave's own offline banner. We keep two independent signals and AND them: `online = browserOnline && fetchOnline`. Either one flipping to false flips the pill immediately; both have to agree online before the pill hides again.
  ...
  - The tracker drives React Query's `onlineManager` from the **browser signal only**, not the combined fetch-failure signal. `onlineManager` gates retry/pause: under `offlineFirst` a query that fails its first attempt has its *retries* paused while `onlineManager` reports offline. If a single transient `fetchOnline=false` (one flaky Firebase read on open) flipped `onlineManager`, that retry would pause — and because `navigator.onLine` never flipped in that case, **no browser `online` event would ever fire to resume it**, wedging the feed on the loading skeleton until a manual refresh. Pausing only on the browser signal is safe because it always has a matching `online` event to resume on; transient fetch failures are left to React Query's own retry/backoff, which is what recovers them. The header pill still reacts instantly to fetch failures via the combined signal (UI-only, doesn't pause queries). Zero new requests — we only instrument ones the app was already making.
