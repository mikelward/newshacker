...
1. **At most three tap zones per row**, always in the same positions: the row body (title + meta) as a single stretched link on the left, a right-side icon button, and room for at most one additional action button on the far-right edge (outboard of the right-side button). No hide, no past, no web, no flag, no inline author link, no rank number, no separate comments button. Phones and library views show only two (row body + right-side button); wide feed rows add a third — a Done button on the far-right edge, with Pin second-from-right (see *Story row layout*). Voting lives on the thread page rather than the row (see *Thread action bar*).
...
Tap zones — there are never more than three: two on phones and on every library view, three on wide-viewport feed rows (where a Done button joins Pin):
...
- **Right-side icon button** — a real icon button on the right, not an inline text link. Has its own 48×48px hit area and ≥8px horizontal gap from the row body. On **feed views** (Top / New / Best / Ask / Show / Jobs / Hot, and the pinned rows prepended to the home feed) this is the **Pin/Unpin** button and tapping toggles pinned state. On **library views** (/pinned, /favorites, /done, /hidden) every visible row already has the state the view is filtered to, so this button carries the view-contextual inverse — orange filled pin / heart / check / visibility-off respectively — and tapping removes the story from that library. See *Library views* below. It's the row's **right-most** control everywhere except wide feed rows, where it sits **second-from-right** and the Done button (below) takes the far edge.

A third tap zone is reserved for at most one additional per-row action. It sits **outboard of the Pin/Unpin button**, on the row's far-right edge — not between the row body and Pin. On **narrow viewports** it stays empty so phones keep the two-tap-zone shape. On **wide viewports (≥960px) feed rows** it materializes as a **Done** icon button (Material Symbols `done`, outline → `check_circle` filled on toggle) **to the right of Pin/Unpin**, so the wide feed row reads row body · Pin · Done (Pin second-from-right, Done right-most) — matching the thread action bar's Pin→Done order. Same toggle semantics as the thread action bar's Done (untoggled → marks done, which also unpins; toggled → unmarks). **All library views** (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) keep this zone empty because the right-side button already names the row's intent — including `/pinned`, where the right-side button stays Pin/Unpin (the view's contextual inverse) rather than swapping for a custom `rightAction`. The library-vs-feed signal is an explicit `isLibraryRow` prop on `<StoryListItem>` that `<LibraryStoryList>` always sets, not the presence of `rightAction` (which `/pinned` doesn't pass). The zone used to sit between the row body and the Pin button (Done to the *left* of Pin); it now sits outboard of Pin so Done is the row's far-right action everywhere, consistent with the thread bar. It was unused before that — voting took it in an even earlier design and has since been moved to the thread page action bar (see below) — and any further use has to clear a high bar: the action has to be something the reader actually wants to do from a scrolling feed without opening the story, not merely something we're capable of adding.
...
Thread page mirrors the same discipline: a single primary "Read article" button at the top of a story view (hidden for self-posts), with a vertical-ellipsis (⋮) **More actions** button leading the icon row, then Upvote (logged-in only), Pin/Unpin, and Done laid out beside it, and a single primary tap target per comment row. See *Comment row layout* below.
...
- **At most 3 tappable zones per story row** — two on phones and library views (row body + right-side icon button), three on wide feed rows where a Done button is appended on the far-right edge (see *Story row layout*). Anything else is display-only. The right-side button is Pin/Unpin on feed views and the view's own "undo" toggle on library views (see *Library views*); on wide feed rows Pin is second-from-right and Done right-most. Upvoting is not on the row today; it lives on the thread page (see *Thread action bar*).
