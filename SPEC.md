...
- Cream background and compact typography reminiscent of HN, with a burnt-orange (`#e65c00`) reserved for the logo mark (a white "n" on the orange tile with a home-indicator pill at the bottom) and accents — never painting the whole header. The default mono / duo presets ship the slightly darker shade rather than HN's exact `#ff6600` because the latter only scores 2.69:1 contrast against the cream banner, and the logo tile read as dissolving into the header on mobile. The opt-in Classic chrome preset still paints `#ff6600` (the whole banner is orange there with white text on top — a different contrast pair that wasn't the problem, and HN-fidelity is the explicit point of that preset). The sticky header uses the page surface tokens (`--nh-bg` / `--nh-text`) so the bar, the body, and the mobile browser URL-bar tint all agree. Tap targets are **fewer, larger, better-spaced** than HN's own mobile site.
...
- Primary color: `#e65c00` (burnt orange, exposed as `--nh-orange`) — reserved for the logo mark (the "n" on an orange tile with a home-indicator pill at the bottom, in the header and the favicon/PWA icon), focus rings, and accents. We picked this shade rather than HN's exact `#ff6600` because the lighter hue scored only 2.69:1 against the cream banner and the logo tile dissolved into the header on mobile; `#e65c00` clears the WCAG 1.4.11 3:1 non-text threshold (tile-vs-banner 3.26:1, white-glyph-vs-tile 3.56:1) and reads as a coherent single brand color across the app. A slightly darker `--nh-orange-dark: #c44e00` is reserved for the pressed/hover state of orange-filled buttons (currently the login submit). The sticky header bar itself uses `--nh-bg` / `--nh-text` (cream / near-black) so the bar, the page body, and the mobile browser's URL-bar tint all read as a single surface. **The Classic chrome preset is the one exception**: under `:root[data-chrome='classic']` the variables flip back to the originals (`--nh-orange: #ff6600`, `--nh-orange-dark: #e65c00`) because Classic explicitly mimics HN's full-orange header bar with white text on top, where (a) the contrast pair is white-on-orange (not orange-on-cream), so the cream-banner contrast story doesn't apply, and (b) HN-fidelity is the entire point of opting in to Classic.
...
- Color contrast ≥ 4.5:1 for body text. The default mono / duo brand orange `#e65c00` hits only 3.56:1 on white, so it's never used for body text — only as the logo tile background behind white glyphs, as an outline color (focus rings, borders), or as the *wordmark* in the Duo preset. WCAG 1.4.3 exempts logotypes from the text-contrast minimum, and "newshacker" in the header is the wordmark side of the logo, not running text — the 3:1 non-text threshold applies instead, which `#e65c00` clears against `--nh-bg` in both light and dark modes (3.26:1 on cream, comfortably above 3:1 on the dark surface). The Classic preset's `#ff6600` only manages 2.69:1 on the cream body for accents like focus rings, which technically rides the 1.4.3 exemption; that's the trade-off Classic users accept in exchange for the legacy HN look (Classic's *header bar* is white-on-orange, a separate pair that's fine).
...
### Install identity
- Web app manifest (via `vite-plugin-pwa`): name "newshacker", theme `#e65c00`, background `#f6f6ef`, `display: standalone`, `start_url: /top`.
