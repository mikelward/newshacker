...
2. **Large, well-spaced hit areas.** Minimum 44×44px per touch tappable (Apple HIG / WCAG 2.5.5 AAA — the project floor), ≥8px dead space between adjacent targets. Story rows sit higher at 48px; the thread action bar drops to 36px under a precise pointer (see *Thread action bar*), by pointer type rather than viewport width. **The floor is a property of the target, not of the ink** — a control may paint smaller than 44px as long as its hit area still measures it (maintainer, 2026-09-03). The comment card's footer icons are the case that uses this: they paint 36px on every device and a pseudo-element restores the missing 8px of height as hit area on touch, so the target stays a full 44×44 while the band no longer sets the card's vertical rhythm at 44. The band's height comes from those buttons rather than from the 13px meta text inside them, so at 44px it left 13px of empty button under the toggle glyph — 17px below a collapsed comment's last ink against 11.75px above its first, which is what read as bottom-heavy. Painting 36 brings that to 12px. The expansion is vertical only: widening it would eat the 2px separating neighboring toolbar buttons, which are three *different* actions.
...
`itemRoot: N`, …) — the census is what shows *what* the cache is full of,
e.g. per-comment entries — those accumulate in memory during a reading
session, but only pinned stories' comments arrive from the persisted
blob (see *Comment bodies persist to disk only for pinned stories*). Sizes, durations, and counts only — never cache
...
2. **Thread view**
   ...
   - **Article summary card** (AI, Gemini 2.5 Flash-Lite) above the action row, for any story with something to summarize — link posts (summarized from the fetched article via Jina) and self-posts (Ask HN / Show HN / text-only, summarized directly from `text`). Auto-runs on load. Stories with neither a `url` nor a `text` body (rare) don't render the card.
     **The summary is a single sentence in the third person, naming its subject** — a claim about the subject rather than a description of the article ("The article argues that…"). It previously asked for the point *in the author's voice*, which reads correctly on the first-person essays HN skews toward but wrong on the news articles and press releases it also carries: there the author is a reporter writing about someone else, so the summary adopted the *subject's* voice and said "We have…" instead of naming who. For a **self-post the submitter is the subject**, so the sentence names them by HN username ("hnuser is looking for a tool that…"), falling back to "the submitter" when the story carries no author rather than letting the model invent a handle. Where a linked article is a first-person account whose writer is never named, the summary may call them "the writer" — same reason.
...
- **React Compiler**: enabled for all builds (default `infer` mode) so components and hooks are auto-memoized — we no longer hand-write `useMemo`/`useCallback`/`React.memo`. `@vitejs/plugin-react` v6 runs the JSX transform through oxc and dropped its `babel` option, so the compiler is wired as a separate `@rolldown/plugin-babel` pass using the plugin's exported `reactCompilerPreset()` helper (see `vite.config.ts`; the wiring is regression-guarded in `vite.config.test.ts`). No `target` override — the default targets React 19 and imports the memo cache from `react/compiler-runtime` (no `react-compiler-runtime` shim). The Rules-of-React ESLint rules (`eslint-plugin-react-hooks` `recommended-latest`, already active) are the compile-time guard that keeps new code compilable; `npm run react-compiler:check` runs the healthcheck (176/176 components compiled cleanly at adoption). The compiler pass is skipped under Vitest (`!isTest` in `vite.config.ts`), so tests assert behavior, not the memoized output. **Cost/reliability:** build-time only — zero runtime cost and no new external calls or services (per CLAUDE.md rule 11). The one measurable cost is a slower build: `@rolldown/plugin-babel` dominates the Babel pass (~67% of build time; local `vite build` went from roughly 3s to ~6s). This lands on CI and the Vercel build, never on the user's device — the shipped bundle is smaller-or-equal and faster at runtime. New failure mode is confined to build time (a future component the compiler can't compile fails the build rather than shipping broken), which the ESLint rules surface first.
...
A third slot is reserved between the row body and the right-side icon button for at most one additional per-row action. On **narrow viewports** the slot stays empty so phones keep the two-tap-zone shape. On **wide viewports (≥960px) feed rows** it fills with a **Done** icon button (Material Symbols `done`, outline → `check_circle` filled on toggle) sitting immediately to the left of Pin/Unpin — same toggle semantics as the thread action bar's Done (untoggled → marks done, which also unpins; toggled → unmarks). **All library views** (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) keep the slot empty because the right-side button already names the row's intent — including `/pinned`, where the right-side button stays Pin/Unpin (the view's contextual inverse) rather than swapping for a custom `rightAction`. The library-vs-feed signal is an explicit `isLibraryRow` prop on `<StoryListItem>` that `<LibraryStoryList>` always sets, not the presence of `rightAction` (which `/pinned` doesn't pass). The slot was unused before — voting took it in an earlier design and has since been moved to the thread page action bar (see below) — and any further use has to clear a high bar: the action has to be something the reader actually wants to do from a scrolling feed without opening the story, not merely something we're capable of adding.
...
> **Forget all writes tombstones, it does not wipe the store.** For the two cloud-synced views (`/done`, `/hidden`) the bulk clear tombstones every live id — the same `{ id, at, deleted: true }` shape a single-row forget writes (see *Sync* above) — rather than emptying the localStorage array. A bare wipe looks correct locally but leaves `/api/sync`'s per-id last-write-wins merge nothing to compare against, so the next pull merges the server's still-live entries back into an empty local list and the whole list reappears: the user's "Forget all" silently undoes itself on a signed-in device. `entryStore.forgetAll()` is the tombstoning bulk op the pages call; `entryStore.clearIds()` remains the hard local wipe and is for local-only resets (tests, sign-out cleanup) only. `/opened` is unaffected — it isn't synced.
>
> **A tombstone is stamped past the entry it replaces, not at `Date.now()`.** Both `mergeEntries` implementations (client `src/lib/cloudSync.ts`, server `api/sync.ts`) accept an incoming entry only when it is *strictly* newer, and `at` is wall-clock time from whichever device wrote it. An entry synced from a device whose clock runs ahead therefore sits in this device's future, and a tombstone stamped at our `now` would lose its own merge — the removal would round-trip straight back off the server. So `removeId` / `removeIds` stamp `max(now, replaced.at + 1)`, per id. This applies to single-row forgets as much as to "Forget all".
...
The **Pin/Unpin** button (Material Symbols `push_pin`, outline → filled on toggle) is the same pinned state as the row-level pin on feeds; label and tooltip flip between "Pin" and "Unpin" based on current state. Reachable from the thread page so stories opened directly from a share link (where there's no feed row to tap) can be pinned and unpinned in the same place. The **Done** button (Material Symbols `done`, outline → filled on toggle) sits immediately right of Pin/Unpin and marks the thread complete: the story is filtered out of every feed (see *Pinned vs. Favorite vs. Done* above) and added to the synced Done list. Tapping Done on a pinned story also unpins it; Done and Pin are mutually exclusive. Tapping **Mark done** also closes the thread, following the same ladder as the browser Back button (`closeArticleView` in `src/lib/`): **(1)** when `window.history.length > 1` the browser has another entry — an in-app feed *or* an external page the reader arrived from (e.g. a deep link opened from Readmo in the same tab) — so `navigate(-1)` back to it; **(2)** otherwise (a lone entry — cold deep link / fresh tab, and unchanged through a `replace`-based redirect) `window.close()` the tab, which dismisses a script-opened tab or an Android Custom Tab straight back into the app that opened us; **(3)** if the browser refuses to close it (notably iOS Safari, or a user-opened primary tab), fall back to the home feed (`/`). We key off `history.length` (which counts real session entries, cross-origin included) rather than `location.key` (non-`default` after a `replace`) or the Navigation API's `canGoBack` (blind to cross-origin entries). So "I'm done" always leaves the thread — back where you came from when that's possible, otherwise closing the tab into the OS's previous window/activity. This mirrors the "mark read" gesture on Apollo-style Reddit clients: the Done tap is "I'm finished, move on". **Unmark done** does *not* navigate — the user is usually on the thread deliberately to revisit a completed item, so yanking them away would be hostile. Browser back is the recovery path for an accidental mark-done; there is no Done toast — the button state (and the story's disappearance from feeds) is the single source of truth, matching Pin.
...
**Right-aligned actions.** `.thread__actions` sets `justify-content: flex-end`, so the icon buttons always hug the trailing edge of the bar. On a URL-backed story the primary/stretch slot's `flex: 1` already consumes the free space, so the alignment is a no-op there; but an **Ask HN / self-post top bar has no primary button** (Read article is hidden), and without `flex-end` the icon buttons (Upvote/Pin/Done/⋮) would fall back to `flex-start` and sit flush-left. The bar reads the same either way — actions anchored to the right.
...
- The comment's vertical padding is asymmetric on purpose — `8px` top / `--comment-stack-gap` bottom. The top edge sits directly above body text and reads best with a comfortable 8px gap; the bottom edge sits below the footer row, whose toggle and toolbar buttons each carry 8px of internal vertical padding inside a 36px band (the touch target is restored past the paint rather than by growing it — see *Design principles* 2). That internal padding already provides visual breathing room below the icons, so a smaller card `padding-bottom` keeps the bottom from reading heavier than the top — symmetric 8/8 made the bottom feel weighted. The band's height is the dominant term, not this padding: while it was 44px even a 4px `padding-bottom` left 17px below a collapsed card's last ink against 11.75px above its first. The gap between any two comment borders — whether the next comment is a sibling or the first nested reply — comes from the same `--comment-stack-gap` token, used by both `.comment`'s padding-bottom (sibling case) and `.comment__children`'s margin-top (nested case), so the two transitions can't drift apart. Tune the rhythm in one place.
...
- **Offline / Down chip** — appears in the header whenever the
  app's evidence-based connectivity tracker (`src/lib/networkStatus.ts`)
  reports something other than `online`. Three states, labeled only from
  evidence: **`offline`** ("Offline", `data-testid="offline-indicator"`) —
  the device reports no network, or a request threw with no response; a
  throw is the *absence* of a response and never blames the backend.
  **`down`** ("Down", `data-testid="down-indicator"`) — the
  backend answered a **5xx on the core data plane** (Firebase reads /
  `/api/items` via `trackedFetch(..., { coreRead: true })`), which is proof
  it is reachable but erroring; a 5xx from a non-critical endpoint
  (summaries, search, telemetry) never flips the whole app. Offline wins
  when both hold. Either chip links to `/offline`, so tapping it opens
  stories already cached on this device instead of acting as a dead status
  label.
  - **No flapping while offline.** The tracker keeps two signals —
    ...
    and ANDs them: either one reporting offline means offline; both must
    agree before showing online, and the OS signal can only ever make the
    app *more* pessimistic. A Workbox-cache-served GET that resolves looks
    like a successful fetch, which would otherwise flip the chip back
    online while the device is genuinely offline (the read path then
    bounces it online↔offline on every cache hit). To stop that, once a
    request fails the tracker is "awaiting liveness": an ambiguous
    (cache-eligible GET-200) success can no longer clear the chip — only a
    **cache-bypassing** success may (a non-GET the server accepted, since
    Workbox runtime caching is GET-only; a GET with a status outside the
    SW's `cacheableResponse` allowlist; or a liveness probe). Recovery is
    ...
    network). Any HTTP response — even 4xx/5xx — proves reachability.
  - **Probe triggers.** Immediately on the offline transition (so a
    transient blip recovers in ~one round trip), every 30s while in doubt,
    on regained tab focus/visibility, when a bounded core read is still
    unsettled after 3s (the **hedge** — lie-fi makes requests hang rather
    than fail, so without it the first offline evidence waits out
    cap + probe ≈ 13s; with it the chip flips in ~hedge + probe ≈ 8s;
    writes/auth are never hedged), and on Network Information API
    `connection change` events (Android Chrome; feature-detected, absent on
    iOS Safari) — the event is ONLY a cue to probe, never truth, and closes
    the no-traffic gap where entering a tunnel goes unnoticed until the
    next user action. Bursts coalesce to one probe in flight.
  - **Read cap + timeout ambiguity.** Core reads are capped at 8s, just
    above the SW's 6s NetworkFirst cache-fallback window (keep
    hedge 3s < SW 6s < cap 8s; ordering regression-guarded in
    `vite.config.test.ts`). A timeout is AMBIGUOUS — offline and
    merely-slow look identical — so it never flips the chip by itself; the
    probe decides.
  - **`down` is sticky against noise.** Sequence counters stale-guard both
    directions: a probe failure that settles after newer success evidence
    can't re-latch offline, and a probe success that settles after newer
    5xx evidence can't clear `down` (health-up ≠ reads-up). Only the
    rate-bounded re-tests — the 30s recovery probe, focus regain, and the
    browser `online` reconnect event (all rare and user-salient) — may
    clear `down` and unpause reads; machine-chatty triggers (hedges,
    connection-change probes) never do. A **cache-bypassing** core-read
    success (a non-5xx status outside the SW's cacheable allowlist) clears it
    too — that *is* reads being up — but a plain GET 200 cannot: it may be
    the SW's NetworkFirst cache fallback answering after its 6s window while
    the real request timed out, which proves nothing about the data plane.
  - **Query-layer pausing.** With the probe configured, React Query's
    `onlineManager` is paused whenever the tracker isn't fully `online`
    (offline OR down), so a struggling backend is never retry-stormed;
    the probe triggers above guarantee the resume path. Without a probe
    (tests/SSR) it falls back to the browser signal only. **Cost:**
    negligible — probes are single tiny GETs, coalesced, mostly while the
    chip is showing, and while genuinely offline they fail without reaching
    the server. No new infrastructure — `/api/me` already exists.
...
- **Undo** (Material Symbols `undo`) — restores the most recent hide action: either the last swipe-to-hide, the last menu "Hide", or the last sweep (the whole batch at once). One level of undo only; recording a new hide replaces the stored batch — **except** that consecutive auto-dismiss-on-scroll hides within a 2s window (`DISMISS_BATCH_WINDOW_MS`) share one batch keyed per burst, so a single Undo restores the whole run the reader just scrolled past. A keyless hide (swipe / Sweep / menu) always replaces the batch, so a manual dismissal between two scroll hides can't be folded into the burst. After restoring, Undo scrolls the list back up to the topmost restored row **when that row is off-screen above the fold** — so undoing a scroll-past burst returns the reader to where they were reading, while undoing a swipe/Sweep (whose rows are still on screen) never jerks the viewport. Disabled when there is nothing to undo. Not persisted across reloads. The undo state is global (lives in `FeedBarContext`) so it survives navigation between list views — a hide on `/top` followed by a jump to `/pinned` keeps Undo armed on the latter's toolbar.
...
- **Appearance** — mode (light/dark/system), app-bar style (mono/duo/classic),
  and text size (a stepper over the px ladder). These are
...
- **Hide stories as you scroll past** (`newshacker:hideOnScroll`). When on, an
  ...
  instantly re-hidden. Undoing such a burst also **scrolls the list back up to
  the topmost restored row** (the earliest one you'd scrolled past) when it
  lands above the fold, so the rows it brought back are actually in view.
...
(mono/duo/classic), and text size (the px ladder) — plus the Home feed
...
- **Tap targets: a ≥44×44px hit area on touch (project floor; story rows sit higher at 48px) — the paint may be smaller where the hit area is restored, as the comment footer's icons do; 36px under a precise pointer for the thread action bar and the comment footer; ≥8px spacing between any two distinct targets.**
...
**Image: static brand icon, not a dynamic render.** `og:image` points at `/icon-256.png` (a 256×256 PWA icon) paired with `twitter:card="summary"` so platforms render the smaller-thumbnail layout instead of letterboxing a square into a wide hero slot. **We use a 256px icon rather than the 512px one because WhatsApp ignores `twitter:card` and picks its layout from the image's pixel dimensions**: a 512×512 square trips WhatsApp's "large enough for a hero banner" threshold and fills a giant preview box (the brand logo blown up huge), whereas a sub-300px image drops it to the compact left-thumbnail card — matching what `twitter:card="summary"` already gives the platforms that honor it (iMessage, Slack, etc.). **256 specifically, not something smaller, because Facebook/Open Graph documents a 200×200 minimum for share images** ([Meta sharing docs](https://developers.facebook.com/documentation/sharing/webmasters/images)); a sub-200px image risks Messenger/Facebook rejecting or omitting the thumbnail. 256 threads the needle — ≥200 for Facebook, <300 for WhatsApp's compact card. `icon-256.png` is generated by `scripts/generate-icons.mjs` alongside the other sizes. Both the static tags in `index.html` and the `/api/og` render carry the 256px image and `og:image:width`/`height` = 256. An earlier iteration prototyped `@vercel/og` to render a story-title-on-cream PNG at 1200×630, but Vercel's Edge bundler doesn't ship the module for non-Next.js Vite projects (it errors with `unsupported modules: @vercel/og` at deploy time). Re-add when a build-time prerender path lands; until then the static brand card is enough. **TODO (follow-up):** for link-type `/item/:id` stories, consider fetching the linked article's own `og:image` (its hero image) and using that as the preview, falling back to the brand icon for self-posts (Ask HN) and articles with no usable image — a relevant hero beats a generic logo, at the cost of an extra crawler-only fetch and new failure modes.
...
**Secondary routes are code-split.** The entry bundle carries only the
surfaces a reader hits on a normal visit — the feeds, `/item/:id`, and
the library views (`/pinned`, `/favorites`, `/done`, `/hidden`,
`/opened`, which share the feeds' components anyway). Everything else
(`/user/:id`, `/settings`, `/search`, `/help`, `/about`, `/login`,
`/offline`, `/debug`, `/admin`, `/tuning`) loads its chunk on first
navigation via `React.lazy`, behind a single `Suspense` boundary whose
fallback is the same `aria-busy` "Loading…" paragraph those pages
already show while their data loads. The operator pages alone were ~8%
of the previous single-chunk bundle, paid by every visitor. The service
worker precaches all chunks, so installed-PWA readers and offline
navigation never pay the extra request; a first-time web reader pays one
same-origin round trip on these rarely-entered routes.
...
| `b` | Go back the way the Back button would (`closeArticleView`): pop `navigate(-1)` to the previous page when one exists (an in-app feed or the external site the reader arrived from), else `window.close()` the tab (dismissing a new tab / Android Custom Tab into the opener), else fall back to the home feed. |
| `?` | Open the keyboard-shortcuts help overlay. |
| `Esc` | Close the menu / overlay (existing behavior). |
...
- **`u` / `b` work on every thread view**, story and focused-comment alike — unlike `o`/`p`/`d`, which are story-only. `b` is browser back (`navigate(-1)`, the same pop mark-done uses); with no back entry it closes the tab, falling back to `/` only when the browser won't close it (see `closeArticleView`). `u` goes up the content hierarchy: on `/item/<commentId>` it navigates to the immediate parent (`item.parent`, a comment or the root story), so repeated `u` walks a deep reply back up toward the story; on a story view it has nowhere higher to go and falls back to `b`.
...
- **Per-comment load failure:** a comment whose fetch fails (or whose id Firebase transiently resolves to null — which happens during HN API hiccups even for ids taken from a live `kids` array) renders a compact "Couldn't load this comment." card with an inline **Retry** button, in the same footprint as the loading card. It must *not* render as a perpetual "…" loading card, and a transient null must *not* be cached as fresh data (`useCommentItem` throws on null so React Query treats it as a retryable error rather than persisting an empty comment for the 7-day staleTime).
...
- **Passive surfaces for updates** (`src/components/AppUpdateWatcher.tsx`, mounted once inside `ToastProvider`):
  ...
  - **`visibilitychange` return-from-hidden → passive ping.** When the tab regains visibility after being hidden for ≥30 s (`pingServiceWorkerForUpdate()` in `swUpdate.ts`), we fire `registration.update()` and forget. If a new SW is discovered it installs + claims via the same `autoUpdate` path, the `controllerchange` listener above shows the toast, and the user gets a non-disruptive nudge when they come back to the tab. Short alt-tabs (under the threshold) don't trigger a ping. The hidden-since timestamp is seeded from the visibility state at mount, so a tab restored already-hidden (browser session restore of a background tab, where no `hidden` event fires after the listener attaches) still pings on its first return.
  - **Periodic passive ping while visible.** A tab kept open and in view but never navigated/PTR'd (an installed PWA, or a parked desktop tab) would otherwise never re-check `/sw.js` — none of the triggers above fire for it. A `registration.update()` every 30 min **while the tab is visible** (the interval is torn down while hidden and re-armed on return) bounds how long such a tab can sit on a stale build; any update found surfaces through the same `controllerchange` toast. Negligible bandwidth — one conditional GET against the tiny `/sw.js` per interval, paused entirely while backgrounded.
  - **Startup ping on mount.** One `registration.update()` fires when the watcher mounts. The browser runs its own SW update check on a cold navigation, but its timing is throttled and an installed PWA the OS killed and relaunched can't rely on it firing promptly — so without this a freshly-opened app could sit on a stale bundle until the 30 min periodic ping comes around. The mount check guarantees a re-check of `/sw.js` at the moment the user opens the app; any newer SW found surfaces through the same `controllerchange` toast (no forced reload). One conditional GET per launch — negligible bandwidth.
- **Stale-entry / stale-chunk recovery** (`src/lib/staleEntryRecovery.ts` + inline boot guard in `index.html`). On the first load after a deploy, the SW can briefly serve the *previous* build's precached `index.html`, which references that build's content-hashed entry (`/assets/index-<oldhash>.js`). That hash is gone from the current deployment, so the entry module fails to load — and because it's the *entry itself* that failed, no React is alive to catch it and the page paints blank until a manual refresh. Recovery is a one-shot auto-reload (what the manual refresh would do) guarded by **two disjoint `sessionStorage` budgets** so it can never loop:
  - **Entry-load failures** (`nh:entry-reload`). A tiny **inline** `<script>` in `index.html`'s `<head>`, placed *before* the Vite-injected entry `<script type="module">` so it runs first and can catch the entry itself (a module can't catch its own load failure). Capture-phase `error` listener; if the failing target is a `<script>`/`<link>` under `/assets/`, it reloads once, keyed on `nh:entry-reload`. The entry module clears that key at its very top (reaching the clear proves the entry loaded), which re-arms the budget for a later upgrade in the same session and tears the inline guard down. Loop-safe: a permanently-broken entry never reaches the clear, so the single reload stands.
  - **Post-boot failures** (`nh:chunk-reload`). `window` listeners installed by the entry for `vite:preloadError` (preventDefault + reload), `unhandledrejection` with a chunk-load signature, and capture `error` for hashed-asset script/link elements — covering dynamic imports / lazy routes / preload. A **separate** budget key, cleared only when a lazy route mounts successfully (`clearChunkReloadBudget`), never on boot (clearing on boot would loop the reload for a genuinely-gone chunk, since the entry always boots fine). Every code-split route's loader (see *Routes* — `lazyPage` in `App.tsx`) clears this budget once its chunk resolves, re-arming the one-shot reload for a later deploy's stale chunk in the same session.
  - Both budgets **fail closed**: if `sessionStorage` is unavailable, no reload fires (an un-recordable reload could loop). The two keys and their clear helpers are independent — spending or clearing one never touches the other.
  - **Rewrite hardening.** Vercel's SPA fallback (`vercel.json`) excludes the hashed-assets path — `/((?!api/|assets/).*)` → `/index.html` — so a *missing* `/assets/*` after a deploy returns a real **404**, not `index.html` (HTML) with status 200. Existing assets are served statically before the rewrite, so this only changes the missing-asset case; without it the worst case is HTML executed as JS. This half helps every client immediately; the inline boot guard only protects an upgrade *from* an index.html that already carries it, so the first fully-protected upgrade is the one after this ships.
...
The SW runtime cache is **additive** to the existing React Query persister (non-pinned stories: 7-day retention in memory, but **comment bodies are excluded from the persisted blob** — see below; pinned: indefinite, comments included). RQ hydrates the UI on cold boot; the SW covers fetches RQ decides to make.

**Comment bodies persist to disk only for pinned stories.** The dehydrate filter (`shouldDehydrateAppQuery` in `useAuth.ts`) drops `['comment', id]` queries from the persisted snapshot unless the pin machinery locked them at `gcTime: Infinity` (see *Pinned cache is never evicted* below — that lock covers top-level and nested comments alike, so the existing invariant doubles as the membership test). The general 7-day comment retention still applies **in memory**, so thread re-opens within a session hydrate instantly; what changed is the disk side: the blob previously grew by one query per comment ever rendered (~7,000 queries / ~7 MB on a real long-lived profile), and boot must read, parse, and hydrate the whole blob before any query may resolve — the measured cost behind "opening a story took 2 s on a reload" — while every throttled snapshot re-serialized all of it. Unpinned comments lost across a restart re-fetch through the normal batch path, usually answered by the SW's HTTP cache, so the trade is a cheap network re-read against a boot tax that grew with every thread ever read. An existing oversized blob self-heals: the first post-deploy boot restores it once, and the next snapshot writes the trimmed set. **Unpinning does not immediately un-persist** a story's comments: the gcTime lock is deliberately monotonic (see *Pinned cache is never evicted* — lowering on unpin was already rejected there as not worth a mechanism), so they keep persisting until the next reload reseats them at the default gcTime and the following snapshot drops them. The bound is one session's pins, and the worst case equals the pre-trim behavior of persisting every comment.

**Persister medium: IndexedDB** (`src/lib/idbPersister.ts`, database `newshacker`, store `rq-persist`, key `newshacker:rq-cache` — the key keeps the old localStorage name so the persisted shape is identical either side of the migration). The persister blob previously lived in localStorage, whose ~5 MB per-origin quota was the binding constraint on how much pinned content could survive offline and whose synchronous writes serialize the whole cache on the main thread on every throttled (1 s) snapshot. IndexedDB's quota is browser-managed (typically hundreds of MB to GBs, same order as the Cache API the SW already relies on) and its writes are async. On first boot the old localStorage blob is **moved** (copied once, then the localStorage key is deleted — freeing the 5 MB budget); if IndexedDB already has a value, it wins and the lingering localStorage copy (a service-worker-cached old bundle in another tab can still write one) is discarded. Engines without IndexedDB — and sessions where `indexedDB` exists but refuses to **open** (storage-blocked webviews, private-mode quirks, where localStorage often still works) — fall back to the localStorage backend under the same key, so persistence continues and a later boot with working IDB migrates; the failed open is cached, so a session stays on one backend, never a mix. A transaction that fails *after* a successful open is instead fail-open (reads → empty cache, writes → no-op — switching that one snapshot to localStorage would leave a newer blob for the next boot's migration to discard): the persisted cache is an optimization, never a correctness boundary, so a broken IndexedDB degrades gracefully instead of crash-looping. Cost: zero — client-side storage only, no new infrastructure or external calls. Reliability: one new local failure mode (IDB unavailable/corrupt), handled by the localStorage fallback and fail-open paths above; worst case is the pre-existing behavior of refetching on boot.
...
- **Thread load** (`useItemTree`): when the root item resolves, warm the first 30 top-level kids via one `/api/items?fields=full` call. A 20-comment thread drops from 21 requests (1 root + 20 items) to 2 (1 root + 1 batch). **The batch is fired, not awaited**: the whole thread UI gates on the root query resolving, so awaiting the batch inside `loadRoot` made every cold open pay two serial round trips before anything but the skeleton painted — now the story header/text/action bar commit after one round trip with comment cards loading beneath. The stampede that await used to prevent (each mounting `<Comment>` firing its own single-item Firebase fetch) is prevented differently: the batch registers each id in an in-flight registry synchronously before fetching, and `useCommentItem`'s queryFn **joins** the registered batch slot instead of fetching singly; a null slot (batch failure or upstream null) falls through to the single-item path, whose throw-on-null retry semantics keep owning the error case. Registry entries live until the batch's cache writes land, so there is no window with neither a slot nor a cached value. Load-more (`Thread.tsx`) and reply expansion (`Comment.tsx`) still wait on the same helper before mounting the next page of observers; the pin/favorite prefetchers were already fire-and-forget inside their own root query functions — no completion boundary existed there to preserve — and their batches now register in the same in-flight map, so pinning from the feed and immediately opening the thread joins the pin-time batch instead of re-fetching.
- **Infinite scroll** (`Thread.tsx` `onLoadMore`): each new page of 20 top-level comments fires another batch for the ids that aren't already cached. Mega-threads stay fast all the way down. **The full set of top-level kids is rendered up front, not just the loaded page** — every kid past the loaded window renders as a fixed-height `<CommentPlaceholder>` (`min-height` ≈ a collapsed comment) below an `IntersectionObserver` sentinel, so the thread's scroll height is established from the total kid count at load time rather than growing page-by-page as the reader scrolls. When the sentinel comes into view it loads (and batches) the next 20, converting that many placeholders into real comments in place. This is why the scrollbar no longer keeps growing as you scroll a long thread; the trade-off is that placeholder height is an estimate, so a real card swapping in still nudges layout slightly — but below the fold, near the sentinel, not by appending at the bottom. Heights are only a guess because comments are variable-height, so the bar is *stable*, not pixel-deterministic. (Nested children are unaffected — they still fetch lazily on expand.) **Pagination is driven by the sentinel's live position, not just an `isIntersecting` edge** (`useInfiniteScroll`): because the full scroll height is reserved up front, a reader on a slow connection can scroll past the 1px sentinel before a page finishes loading; a plain `isIntersecting` check would then never fire again (the sentinel sits above the viewport and never re-enters), permanently stranding the rest of the thread on gray placeholders. Instead, an intersection (or the sentinel landing above the viewport top) kicks off a catch-up loop that keeps loading pages — re-measuring `getBoundingClientRect().top` against the viewport height after each page (and after a frame, so the freshly revealed comments are laid out) — until the sentinel drops back below the fold. The live measurement is deliberately *not* taken from the observer entry, whose `rootBounds` is offset ~400px above the real viewport by the prefetch `rootMargin`. A normal in-view prefetch loads exactly one page (the sentinel is still below the fold after it), so a visible sentinel never dumps the whole thread at once.
...
The helper is best-effort — on failure (`/api/items` 5xx, offline at pin time) the per-comment `useCommentItem` falls back to individual Firebase fetches, so nothing breaks visibly. Two failure-isolation rules keep one bad upstream minute from snowballing:

- **The batch fetch carries an 8-second deadline** (`COMMENT_BATCH_TIMEOUT_MS`, via `AbortSignal.timeout`). Thread's infinite-scroll `onLoadMore` (and `Comment.tsx` reply expansion) wait on the batch before mounting the next page of `<Comment>`s, so without a deadline a hung `/api/items` request left the reader staring at placeholder skeletons until the browser's own fetch timeout (often minutes). `loadRoot` fires the batch without awaiting it (see *Thread load* above), so for the thread-load batch the deadline instead bounds how long mid-batch observers wait on their joined registry slots before falling back. On abort the prefetch is swallowed like any other failure and the per-comment fallback takes over.
...
- As a row with `score > 100` **scrolls fully into the viewport**, `StoryList` calls `prefetchFeedStory` (in `src/lib/feedStoryPrefetch.ts`). It delegates to the same `prefetchPinnedStory` used at pin-time, so the warm shape is identical: `['itemRoot', id]`, the first 30 top-level comments (one shared `/api/items?fields=full` batch), the article AI summary, and the comments AI summary. Tapping a popular headline renders the thread, summaries, and early comments without a round-trip.
- **The viewport gate is what bounds this, and it was the home page's single biggest cost.** The warm used to fire for every row of the *loaded page* that cleared the score threshold, not every row on screen — and on `/top` nearly every front-page story clears 100 points, so opening the app fired all 30 at once. `prefetchPinnedStory` fetches each root with a **single-item** Firebase read (`getItem`), so that was 30 direct HN requests, plus a comment batch and two summary calls apiece, all issued in the same moment as the feed's own id-list and items requests and competing with them for the connection. Measured against a stubbed backend at 6× CPU throttle, moving the gate to the viewport took a home-page load from ~143 requests to ~27 and first paint from ~3.9 s to ~2.8 s. A drive-by warm only pays for itself on a row the reader can tap, and a row below the fold is not one of those — so this now uses the same `inViewIds` signal *Warm-on-view server summary cache* below already used. The cost is a reader who taps within a few hundred ms of a row entering the viewport: the warm is in flight rather than complete, and the thread opens on a normal fetch. `StoryList.warmTrending.test.tsx` guards the gate.
- Tracked per-session via a `Set` in `StoryList` so a row re-entering the viewport doesn't re-fetch, and `prefetchFeedStory` short-circuits outright if `['itemRoot', id]` is already cached.
...
- **What it does.** `/api/warm-summaries` is a Vercel cron that runs every 5 minutes against `?feed=top&n=30`, fetches the feed, takes the first N eligible ids (`score > 1`, not dead/deleted), and for each one runs **two independent tracks in parallel**:
  - **Article track.** For link posts, re-fetches the article via Jina Reader (markdown), SHA-256 hashes it, compares against `articleHash` on the stored record. Unchanged → bump `lastCheckedAt`. Changed → regenerate the one-sentence summary via Gemini and overwrite the record. Jina is a hard dependency for the link-post path — there's no server-side raw-HTML fallback (see TODO.md § "Article-fetch fallback" for the rationale). For self-posts (no `url`, body in `text`), the track skips Jina entirely and hashes the stripped plain-text body itself; Gemini is then prompted with a self-post-specific variant that references the post title and the submitter's username, and asks for their claim / question directly (see *Thread view* for the third-person rule both prompts share). Stories with neither `url` nor `text` log `skipped_no_content`.
...
- **Shared cache records** (Upstash JSON, 30-day TTL, written by both user-facing handlers and the cron):
  ...
  Legacy pre-schema entries (plain summary string / bare `string[]` of insights) are treated as absent on read and silently overwritten by the next regeneration. The user-facing endpoints return any present record unconditionally — the cron owns freshness — but a store read *error* (Redis unreachable), or a cache miss while the write breaker is open (a recent `store.set` failed), returns 503 `store_unreachable` rather than generating (the circuit breaker; see the Upstash reliability note).
...
- **Cost/reliability (rule 11).**
  ...
  - **Gemini.** Fires only on actual content change (article-hash for articles, transcript-hash for comments). Articles churn ~10–30% of ticks; comments churn more on young threads but settle once HN-rank reshuffles stop (and the min-kids gate stops us from regenerating thin-thread noise). **This was estimated at ~$3–5/month realistic / ~$15 worst-case, and production measurement says otherwise.** `reports/2026-04-29-cache-strategy.md` censused 24 hours of cron-only Gemini usage on this same configuration: **5,440,518 prompt + 31,283 output tokens/day** (article 5.04M/16.6K, comments 398K/14.7K — the article track is 12.7× the comments track on prompt tokens). At the $0.10/M input, $0.40/M output rates recorded for flash-lite elsewhere in this document, that is **$0.557/day, ~$17/month** — above what was called the worst case. Note the report computes at $0.075/$0.30 and flags that rate as unconfirmed; at its pair the same census is ~$12.50/month. Budget on the higher, confirm against Google's current published pricing. Over half of the article-track spend is regeneration on sub-noise content deltas that `WARM_MIN_DELTA_BYTES` (specified in that report, never shipped) would remove.
  - **Upstash.** Two keys per story instead of one, and `processStory` reads *both* for every selected id on every tick, unconditionally — the backoff gate runs on what those reads return, so it cannot save the read. That is 288 ticks/day × 30 stories × 2 GETs = **17,280 commands/day, ~520k/month**, before record writes, the rate limiter's `INCR`/`EXPIRE`, telemetry `LPUSH`/`LTRIM`, or user-facing summary reads. This bullet previously said "well under free-tier quotas"; that was wrong — it is roughly 1.7× the 10k commands/day free allowance cited elsewhere in this document. At Upstash's pay-as-you-go rate (~$0.20 per 100k commands) that baseline is **~$1/month** — minor, but a real line rather than a free one, and it rises with reader traffic since warm-on-view adds two reads per visible row. **The reader write-probe** (below) adds one `SET` per cache miss that reaches generation — placed after the eligibility and rate-limit gates, so rejected/over-quota requests issue none. A miss recurs only until the 30-day record caches, so probe volume tracks *distinct summaries generated*, not visits: one extra command per generation, negligible against the read baseline above and comfortably inside pay-as-you-go (~$0.20/100k). It does not move the free-vs-paid decision — the cron read baseline (~1.7× the free allowance) already does. **Reliability — the circuit breaker.** An unreachable Redis now fails **closed**, not open. Both stores' `get` let a Redis error propagate; `processStory` distinguishes a read *error* from a genuine miss and skips **both** tracks with `skipped_store_unreachable`, generating nothing. The user-facing `/api/summary` and `/api/comments-summary` likewise return **503 `store_unreachable`** on a read error instead of falling through to generation. This is deliberate: a summary generated during an outage can neither be cached (the write fails too) nor rate-limited (the limiter shares this Redis and fails open), so it is thrown away and regenerated on the next request/tick — unbounded, throwaway Gemini spend. **This replaced the earlier fail-open behavior** (`get` did `.catch(() => null)`, a null record was indistinguishable from a never-seen one, so every eligible story took the `first_seen` path and regenerated each tick), which turned a real Upstash outage into an unbounded Gemini bill — a runaway that `WALL_CLOCK_BUDGET_MS` could not bound (it caps how many stories a tick *starts*, not what the in-flight ones spend, and nothing bounded the damage across ticks). **A store that serves reads but rejects writes** (Redis at `maxmemory` with `noeviction`, a quota'd tier that reads free but bills writes) is the harder case the read-side breaker misses: the read is a genuine miss, generation succeeds, the write fails, and nothing is ever cached — a *per-request* runaway, not a one-off race. A **write breaker** closes it: a failed `store.set` opens a module-scoped breaker (60 s cooldown, per warm instance), during which a cache miss fails closed — **503 `store_unreachable`** on `/api/summary` and `/api/comments-summary`, `skipped_store_unreachable` on the cron — instead of generating. On the **per-request reader endpoints** the breaker's cooldown alone was not enough: it only *allows* a retry after 60 s, so a store that keeps rejecting writes would resume generation optimistically and drip one generation per cooldown per warm instance (and one per cold instance, whose module state starts with the breaker closed). So the reader miss path **probes writes before generating** (`SummaryStore.probe`, one `SET`/`EX` on a **fresh key each call** — so an allocation rejection under `noeviction` fails it exactly as a new record would — the same pattern as the cron): on a miss it proves a write succeeds before paying for Jina/Gemini, and on probe failure it re-opens the breaker and fails closed. A sustained write-only outage therefore generates *nothing* for every failure the probe can observe — Redis unreachable, an auth failure, a command-count quota, or a `maxmemory`/`noeviction` allocation rejection — leaving only cheap probe writes. **Two residuals remain, both bounded by the write breaker** (the failed real `set` still trips it) to one generation per cooldown per instance — never the per-request runaway: a same-instant race (concurrent misses whose probes all succeed just before Redis starts rejecting the real `set`, each generating once), and a strictly size-proportional byte quota with room for the tiny probe but not the KB record (implausible for summary-sized records). Closing either fully — global request serialization for the race, a record-sized probe that still could not guarantee the *next* variable-size record fits — is not worth the cost for these edges (`TODO.md` → *Decisions needing review*). The **cron** needs the probe up front for a different reason: it runs up to `CONCURRENCY` (5) stories × 2 tracks concurrently and a per-story breaker check can't bound that first wave (all ten snapshot it closed before the first `set` rejects), and the 60 s cooldown is shorter than the 5-minute tick so it never carries forward. So the cron **probes writes once before the pool** (`SummaryStore.probe` — one sentinel `SET`/`EX` on the shared Redis): if the probe throws, the breaker is tripped before any worker starts, so every story in the tick skips `skipped_store_unreachable` with nothing generated. The per-story breaker then remains only as the mid-tick safeguard for a write that starts failing *after* a healthy probe — worst case one concurrency window before the queued remainder skips. Deploying with **no** Upstash configured (`store === null`) is a separate, deliberate no-cache mode and is unaffected by the breaker.
...
- Favoriting a story calls `prefetchFavoriteStory` — same shape at warm time, but with one deliberate difference in what survives a restart: a favorite's **root and both AI summaries** persist (they ride the normal 7-day window and are small), while its **comment bodies are session-only** — the pinned-only comment-persistence rule (see *Comment bodies persist to disk only for pinned stories*) applies to favorites too, on purpose. Favorites can arrive in bulk via HN sync (hundreds at once), and persisting every favorite's comments would rebuild exactly the unbounded blob that rule exists to prevent; the offline-reading guarantee is Pin's job (CLAUDE.md rule 9 — Pinned is the reading list, Favorite the keepsake). Offline, a favorited-but-unpinned story shows its title/text and both summaries — the comments summary carries the discussion's gist — and pinning it is the one-tap upgrade to full offline discussion.
...
- **Evidence-based connectivity tracker** (`src/lib/networkStatus.ts`). `navigator.onLine` on mobile lags badly behind reality — walking into a tunnel can leave it stuck at `true` for tens of seconds (sometimes it never flips, and it can flicker), so the header pill would appear long after Brave's own offline banner. We keep two independent signals and AND them: `online = browserOnline && fetchOnline`. Either one flipping to false flips the pill immediately; both have to agree online before the pill hides again, and the OS signal can only ever make the app more pessimistic — it can never prove we're online.
  - `fetchOnline`: every app fetch goes through `trackedFetch`, which flips `fetchOnline` to false the instant a request throws a hard network error (`TypeError` / "Failed to fetch" / `NetworkError` and the equivalent strings other runtimes use) and back to true the instant any response comes back (even a 500 proves we reached a server). `AbortError` is ignored so a superseded query doesn't masquerade as a connectivity drop, and our own read-cap `TimeoutError` is ambiguous — it triggers a probe instead of flipping anything.
  ...
  - A third state, **`down`** (backend-unreachable), sits on top of the offline axis: a 5xx on the core data plane (reads marked `coreRead: true` — Firebase feeds/items and `/api/items`) is proof the backend is reachable but erroring. A thrown fetch is the absence of a response and never blames the backend; a 5xx from a non-critical endpoint never flips the whole app. See the *Offline / Down chip* section under *Header* for the full probe/hedge/stale-guard design.
  - With the probe configured (`/api/me`, set in `main.tsx`), the tracker pauses React Query's `onlineManager` whenever it isn't fully `online` — offline **or** down — so a struggling backend is never retry-stormed. Pausing on fetch evidence used to be unsafe (a transient `fetchOnline=false` with no matching browser `online` event wedged the feed on the loading skeleton); it is safe now **only because** the probe machinery guarantees a traffic-free resume path: an immediate probe on the offline transition (a transient blip recovers in ~one round trip), the 30s recovery interval, focus/visibility probes, and browser `online` events. Without a configured probe (tests / SSR) the old rule stands: `onlineManager` follows the browser signal only, and the pill reacts to fetch failures UI-only.
  - Automatic retries are bounded (exponential backoff + jitter on the feed queries) and only ever follow **true statusless network blips** (a thrown fetch, a read-cap timeout) — never a response that carried an HTTP status: a 4xx won't change on retry, and retrying a 5xx storms a backend that just said it's struggling (the `down` state + rate-bounded recovery probe own that path). See `isRetryableFetchError`, `feedQueryRetry` and the app-wide `retry` default in `main.tsx`.
