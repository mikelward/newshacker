...
4. **Navigation & Chrome**
   - Sticky header with the orange "ñ" keycap mark, wordmark, and current feed name.
...
- Primary color: `#ff6600` (HN orange) — reserved for focus rings and accents. The logo mark (the "ñ" on a rounded-square "keycap" tile, in the header and the favicon/PWA icon) uses a slightly darker `#e65c00` so it reads as its own mark rather than blending into every orange accent on the page. The sticky header bar itself uses `--nh-bg` / `--nh-text` (cream / near-black) so the bar, the page body, and the mobile browser's URL-bar tint all read as a single surface.
...
### Install identity
- Web app manifest (via `vite-plugin-pwa`): name "newshacker", theme `#e65c00` (matches the logo tile so the OS chrome around the installed app reads as an extension of the mark), background `#f6f6ef`, `display: standalone`, `start_url: /top`.
- Icons (generated once by `scripts/generate-icons.mjs`, checked into `public/`): `icon-192.png`, `icon-512.png`, `icon-512-maskable.png`, `apple-touch-icon.png` (180), `favicon.svg`, `favicon-maskable.svg`, `favicon-32.png`. The mark is an `#e65c00` rounded-square "keycap" tile with a white "ñ" centered on it — the tilde nods at the language-quirk side of "hacker" and makes the mark impossible to confuse with HN's `Y` logo. The maskable variant is rasterized from a separate `favicon-maskable.svg` that uses a full-bleed orange tile (no rounded corners, since the launcher applies its own shape mask) and pulls the glyph into the 80% adaptive-icon safe zone so aggressive OEM crops can't clip the tilde.
