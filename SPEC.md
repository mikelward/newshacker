...
**newshacker** is a mobile-friendly, responsive web **reader for [Hacker News](https://news.ycombinator.com)**, built with React + TypeScript and deployed on Vercel. The goal is to deliver a clean, fast, thumb-friendly reading experience with a familiar orange brand mark, while trimming the interface down to the features that matter on a phone.
...
- Familiar reader look & feel — orange `#e65100` (Material Orange 900) header, white background, compact typography — but with **fewer, larger, better-spaced** tap targets than HN's own mobile site.
...
- Primary color: `#e65100` (Material Orange 900) for the header and accents. Darker hover shade `#bf360c` (Material Deep Orange 900) exposed as `--nh-orange-dark`. Orange is identical in light and dark mode — we don't dim the brand on dark.
- Surfaces and borders are neutral grey in both modes — light mode is `#ffffff` page / `#f5f5f5` code / `#e0e0e0` border, dark mode is `#1b1b1b` page / `#222222` card / `#2a2a2a` code / `#2f2f2f` border. The previous HN-cream `#f6f6ef` (light) and warm-tinted darks (`#1b1b17`, `#2a2a25`, `#2f2f2a`, etc.) were dropped so both themes share the same neutral-gray + orange logic and feel unified.
- Text: `#000` primary, `#5a5a5a` read/opened titles, `#828282` metadata in light mode; `#ececec` / `#c4c4c4` / `#9a9a9a` respectively in dark mode. The opened-title color sits between primary and meta so a row the reader has already opened is clearly de-emphasized without fading into the meta line below it.
...
- Color contrast ≥ 4.5:1 for body text (brand orange on white fails for small text — only used on large headers / buttons).
...
### Install identity
- Web app manifest (via `vite-plugin-pwa`): name "newshacker", theme `#e65100`, background `#ffffff`, `display: standalone`, `start_url: /top`.
