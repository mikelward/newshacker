...
- **Summary cache warming** (`/api/warm-summaries`): never cached (`no-store`). Fire-and-forget POST from the feed as rows approach the viewport, one batch per ~200 ms debounce; the endpoint applies filters, a 5-min per-id dedup, a per-IP rate limit, and a daily Gemini budget cap before delegating to `/api/summary`. Triage counters at `/api/warm-stats`. See `SUMMARIES.md` § "Server-side warming on visibility" for the full shape, constants, and error taxonomy.
