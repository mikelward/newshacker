...
- **Tiered backoff (the knobs these analytics exist to tune).** The article and comments tracks have diverged: articles use a flat fresh/stable split, comments use a doubling-width ladder keyed off HN `story.time`. Bucket widths match the analytics `ageBand` axis below so the "when do polls stop being worth it" question is answered by the same histogram on both sides.
  ...
  - `WARM_MIN_DELTA_BYTES` (default **256**): the delta guard. When the article body hash differs but the byte length moved by fewer than this against the last regeneration, treat it as dynamic-page noise (rotating timestamps, ad slots, related-items widgets) and skip the Gemini call, logging `skipped_minor_delta`. The record is preserved except `lastCheckedAt`, so a *monotonic* drift (a growing in-body counter) accumulates against the fixed baseline and eventually crosses the threshold. `reports/2026-04-29-cache-strategy.md` measured ~60% of article `changed` ticks as sub-100-byte noise — over half of cron Gemini spend. It is a byte-length proxy, not a content diff, with two accepted misses: a short real edit under the threshold, and — because the byte delta stays near zero — a same-length full rewrite, which is caught only by a later larger edit or the max-age cutoff, not by accumulation. Both are the deferred content-aware gate's job: the correction/lede/title signals are logged on the skip line as its input (`TODO.md`). *Link posts only* — the noise it absorbs is a property of Jina-fetched external pages; a self-post body comes straight from HN `text`, so any change there is a real author edit and always regenerates. *Comments track is left ungated too — its kids accumulate continuously, so a change is real growth, not noise.*
...
- **Cost/reliability (rule 11).**
  ...
  - **Gemini.** Fires only on actual content change (article-hash for articles, transcript-hash for comments). Articles churn ~10–30% of ticks; comments churn more on young threads but settle once HN-rank reshuffles stop (and the min-kids gate stops us from regenerating thin-thread noise). **This was estimated at ~$3–5/month realistic / ~$15 worst-case, and production measurement says otherwise.** `reports/2026-04-29-cache-strategy.md` censused 24 hours of cron-only Gemini usage on this same configuration: **5,440,518 prompt + 31,283 output tokens/day** (article 5.04M/16.6K, comments 398K/14.7K — the article track is 12.7× the comments track on prompt tokens). At the $0.10/M input, $0.40/M output rates recorded for flash-lite elsewhere in this document, that is **$0.557/day, ~$17/month** — above what was called the worst case. Note the report computes at $0.075/$0.30 and flags that rate as unconfirmed; at its pair the same census is ~$12.50/month. Budget on the higher, confirm against Google's current published pricing. Over half of the article-track spend was regeneration on sub-noise content deltas; the `WARM_MIN_DELTA_BYTES` delta guard (specified in that report) now skips those, logging `skipped_minor_delta` — expect the article `changed` count, and the cron's Gemini prompt tokens, to drop by roughly half. Confirm the drop against 24–48 h of post-deploy `warm-story` logs.
  - **Upstash.** Two keys per story instead of one, and `processStory` reads *both* for every selected id on every tick, unconditionally — the backoff gate runs on what those reads return, so it cannot save the read. That is 288 ticks/day × 30 stories × 2 GETs = **17,280 commands/day, ~520k/month**, before record writes, the rate limiter's `INCR`/`EXPIRE`, telemetry `LPUSH`/`LTRIM`, or user-facing summary reads. This bullet previously said "well under free-tier quotas"; that was wrong — it is roughly 1.7× the 10k commands/day free allowance cited elsewhere in this document. At Upstash's pay-as-you-go rate (~$0.20 per 100k commands) that baseline is **~$1/month** — minor, but a real line rather than a free one, and it rises with reader traffic since warm-on-view adds two reads per visible row. **Reliability:** the reads are `.catch(() => null)` and a null record is indistinguishable from a never-seen one, so an unreachable or quota-exhausted Redis loses the age / interval backoff — the one gate that reads the record — and every *eligible* track takes the `first_seen` path and regenerates each tick, converting the cheapest outcome into the most expensive one. Not *every* track: the record-independent gates sit after the backoff block and still fire, so a dead/deleted story, `score ≤ 1`, a story with neither URL nor body, and a Jina failure still log `skipped_unreachable` / `skipped_low_score` / `skipped_no_content` / `skipped_payment_required` (`api/warm-summaries.ts:1444-1472`), as do the comments track's own content gates (`:1732-1765`). The min-kids gate gets *stricter*, not looser — it's written `if (!existing && …)`, so an outage pushes thin threads into `skipped_low_volume` that a live record would have let through. The outage signature to look for is therefore an anomalous `first_seen` spike **among eligible tracks**, not universal regeneration; the spend is bounded by how many of the selected stories were eligible that tick, which is why this is a real bill but not the worst case the sentence used to imply. `WALL_CLOCK_BUDGET_MS` (50 s) is weaker protection than it sounds: the elapsed check runs inside the pool worker *before starting* each queued story (`api/warm-summaries.ts:1993`), so it stops new work from beginning but cannot bound the up-to-`CONCURRENCY` stories already in flight — and the Gemini call takes no timeout of its own (the only `signal` threaded through is `request.signal`, i.e. a Vercel-level abort). So it bounds how many stories a runaway tick *starts*, not how long that tick runs or what it spends, and nothing bounds the damage across ticks.
