...
2. **Thread view**
   ...
   - **Article summary card** (AI, Gemini 2.5 Flash-Lite) above the action row, for any story with something to summarize — link posts (summarized from the fetched article via Jina) and self-posts (Ask HN / Show HN / text-only, summarized directly from `text`). Auto-runs on load. Stories with neither a `url` nor a `text` body (rare) don't render the card.
     - **Archive-link comment hoist (paywall fallback).** When the first
       Jina pass on a link post comes back looking like a paywall
       overlay (the same `detectPaywall` heuristic used for telemetry —
       JSON-LD `isAccessibleForFree:false`, marker phrases, etc.), the
       handler walks the top 20 top-level comments and looks for an
       allowlisted archive URL: `archive.org`, `web.archive.org`,
       `archive.ph`, `archive.today`, `archive.is`, or
       `ghostarchive.org` (exact host match — no subdomains, no other
       hosts). If a comment from a non-deleted, non-dead author
       contains one, the handler re-runs Jina against the archive URL
       and summarizes that body instead. Cached under the *original*
       story id alongside provenance (`archiveSource`:
       `{ archiveUrl, archiveHost, username }`). The card footer
       becomes "Article summary generated by Gemini from `<host>`
       copy linked by HN user `<username>`" so the reader knows where
       the bytes came from. Failure modes are non-fatal: if no
       comment links to an allowlisted archive, or the archive Jina
       fetch fails (Cloudflare challenge on archive.ph etc.), we fall
       through to today's behavior and summarize the original
       paywall body. The same scan-and-re-fetch logic runs in the
       `warm-summaries` cron so cached records and re-checks agree on
       the archive copy.
