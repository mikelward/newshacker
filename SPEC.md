...
4. **Navigation & Chrome**
   - Sticky header with the orange "n" tile mark (speaker-slot variant), wordmark, and current feed name.
...
- Primary color: `#ff6600` (HN orange) — reserved for the logo mark (the "n" on an orange tile with a small speaker slot above, in the header and the favicon/PWA icon), focus rings, and accents. The sticky header bar itself uses `--nh-bg` / `--nh-text` (cream / near-black) so the bar, the page body, and the mobile browser's URL-bar tint all read as a single surface.
...
- Icons (generated once by `scripts/generate-icons.mjs`, checked into `public/`): `icon-192.png`, `icon-512.png`, `icon-512-maskable.png`, `apple-touch-icon.png` (180), `favicon.svg`, `favicon-32.png`. The mark is an orange rounded-square tile with a white "n" centered on the face and a small white speaker slot above — a subtle nod to a phone earpiece, since newshacker is mobile-first. At favicon scale the speaker slot aliases away and the mark reads as an "n" on an orange tile; at 192/512 px the slot is visible. Can never be mistaken for the HN `Y` logo. The maskable variant is rasterized from the same SVG; the glyph + slot sit well inside the 80% adaptive-icon safe zone.
