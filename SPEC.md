...
Every scrolling list view — feed pages (`/top`, `/new`, `/best`, `/hot`, etc.) and library pages (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) — ends in a **bottom action bar** styled after the thread page's bottom action bar: a single horizontal flex row on `.story-list__footer`, 12px gap, wrap to a second row on narrow screens. The thread page has its own version of this bar (see *Thread action bar*); the list-view bar mirrors its layout conventions.

**Slots, left → right, always in this order:**

1. **Back to top** (left). Same button as the thread page's bottom-bar Back to top — Material Symbols `vertical_align_top`, neutral styling, calls `window.scrollTo({ top: 0, behavior: 'smooth' })` (browsers short-circuit this to an instant scroll when the user has `prefers-reduced-motion: reduce` set). Always rendered. It sits leftmost to match the thread bottom bar's leftmost Back to top, so a reader who has learned the thread bar already knows where this button is. Implemented by the shared `<BackToTopButton>` component; testid `back-to-top` on feeds and library pages alike. **Variant differs by footer:** on feed footers it renders icon-only (56×56 square, tooltip + `aria-label`-driven accessible name) so the More button — which stretches to fill the middle slot — sits visually centered between two same-sized squares (Back to top on the left, Hide unpinned on the right). Library footers render the labeled variant (icon + "Back to top" text, full-width via `flex: 1`) because Back to top is the bar's only button there and a labeled full-width target reads better than an icon stranded on the left edge.
...
**Sizing — More is the one that stretches.** On the feed footer (`.story-list__footer--feed`), More is the action a returning reader came for ("give me the next page of stories"), so it's the one that grows to absorb the row's free space and becomes the biggest target. Back to top renders icon-only here (56×56 square, no label) so More sits visually centered between two same-sized squares. Hide unpinned stays at its fixed 56×56 square, right-pinned via `margin-left: auto`. When the feed has exhausted its id list and More is absent, Back to top stays a 56×56 square on the left and `margin-left: auto` on the sweep button takes over the right-pinning on its own — there's no labeled full-width fallback, since the icon-only Back to top is now permanent on feed footers. Library footers don't carry the `--feed` modifier, so Back to top renders in its labeled variant and grows to fill the row there (it's the only button — a full-width target beats a small one stranded on the left edge).

**Depth caption above More.** Feed pages render a small muted caption immediately above the action bar — `Loaded N <feed> stories` (for example `Loaded 30 top stories`). `N` is the number of stories loaded from that feed page stack so far (30 per page), not the currently visible row count; pinned/hidden/done/score filters don't change it. It's a passive signal that lets the reader notice they're at story 120 before they tap More again, without overloading the More label itself (which would read as a property of the *next* page, not what's already on screen). The caption renders whenever the More button renders, and only then — they share the same `hasMore` gate. When every loaded story is filtered (e.g. heavily-hidden `/new`) the caption still reads from loaded depth (`Loaded 30 new stories`) so the reader sees progress rather than a misleading `0`. Feed-only; library pages never render either widget. Class `.story-list__count`, testid `story-list-count`.
