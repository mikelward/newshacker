...
1. **Story feeds**
   ...
   - **Feed refresh status (opening after a while).** The persisted React Query cache paints the last-seen list instantly on open, then the feed refetches in the background **when the cached list has gone stale** — every refetch trigger (mount, tab refocus, reconnect) is gated on the app-wide 5-minute `staleTime` (the cache TTL), so opening or refocusing a feed whose snapshot is older than that picks up new stories, while navigating back to a list you were reading seconds ago stays quiet instead of re-checking. (Pull-to-refresh and **More** bypass the TTL to force a check.) The mount trigger deliberately uses React Query's stale-gated `refetchOnMount: true`, *not* `'always'`: the earlier `'always'` ignored the TTL and re-checked on every remount — since opening a story unmounts the feed, that flashed the refresh strip on every "back", far more often than readers expected. **Opening a story also kicks a stale-gated background refresh of the feed** (`FeedItemsState.refreshStale`, fired from `handleOpenThread` — but only on a *same-tab* open; a modified/middle click that opens the thread in a background tab keeps the reader on the still-mounted feed, so the refresh is skipped there to avoid reordering rows out from under them): if the list has passed its TTL it refreshes *while the reader is in the thread*, so returning shows the current ranking with no strip and no wait; a still-fresh list is left untouched, so this adds no fetch beyond what the TTL already permits. This is gated on the TTL rather than firing unconditionally on every open on purpose — an unconditional refresh would re-fetch on every story tap (heavy item-proxy traffic for active readers) for a marginal freshness gain over the TTL-gated version. **This refresh depends on `<FeedKeepWarm>`** (`src/components/FeedKeepWarm.tsx`, mounted once above the router): React Query aborts a query's in-flight fetch the moment its *last* observer unmounts, and opening a story unmounts the feed — so without a second, persistent observer the on-open refresh would be cancelled before it lands. `FeedKeepWarm` subscribes to the most-recently-viewed feed's queries (tracked in `src/lib/activeFeed.ts`, set by the feed views on mount) using the same `useFeedItems` / `useHotFeedItems` hooks, so the refresh survives navigation. While the feed route is also mounted the two observers share the query and React Query dedupes their fetches, so keeping-warm costs nothing there; the trade-off is that the last-viewed feed stays subscribed (and so can refetch on window-focus when stale) even while the reader is on a thread or another page — deliberate, since that is exactly the freshness this feature wants. A thin status strip at the foot of the list — just above the More / Back-to-top row, so the load-related affordances sit together — reports that background refresh so a stale snapshot is never shown silently: a quiet *"Checking for new stories…"* (with spinner) while the refresh is in flight, and a *"Couldn't load new stories."* row with a **Retry** button when it failed. The strip only appears while rows are already on screen — the first-ever load still owns the loading skeletons / error / empty states. The id-list query (`['storyIds', …]`, the one read that hits Firebase directly rather than the `/api/items` proxy) retries a few times with exponential backoff + jitter (configured via `setQueryDefaults` in `main.tsx`; only statusless network blips retry — never a response that carried an HTTP status, see `feedQueryRetry`) before the failed state shows, so a momentary blip on open self-heals; the "More" page fetch keeps its deliberate bail-on-failure behavior and is not retried. The detection of "showing stale rows because the background refresh failed" relies on the fact that React Query keeps a query's status at `success` (so `isError` stays false) when it already has data and only a refetch fails — see `deriveRefreshState` in `src/hooks/useStoryList.ts`. The `/tuning` Preview opts out of the strip. Cost / reliability: no new network beyond the refetch that already happened; the retry is bounded and scoped to the tiny id-list JSON, so no extra item-proxy or summary spend.
