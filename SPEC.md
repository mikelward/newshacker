...
- **Comment-card expand/collapse icon (all devices).** The card's bottom-right corner carries a Material Symbols `expand_circle_down` (collapsed) / `expand_circle_up` (expanded) icon — a circled directional chevron, semantically "expand this", not to be confused with plain `expand_more` / `expand_less` chevrons which read as "next". The icon is pushed to the right edge of the meta row via `margin-left: auto` on `.comment__toggle-icon` (the toggle button itself is `flex: 1 1 auto` so there's free space for the auto-margin to consume). Visible on every device so the expand/collapse control is obvious regardless of whether the reader taps the card body or aims for the icon. The icon is inside the same `<button>` that wraps the meta text, so the whole row stays one tap zone per the *Comment row layout* rule.
