...
- **Share** — invokes the Web Share API (or copies the link to the clipboard as a fallback) via the `useShareStory` hook. Shares the article URL when the story has one, falling back to the `/item/:id` thread URL on self-posts — so it's always available. On wide viewports this is an inline icon on the bar; below 960px it lives here in the overflow.
