...
Thread page mirrors the same discipline: a single primary "Read article" button at the top of a story view (hidden for self-posts), with Upvote (logged-in only), Favorite, Done, and a vertical-ellipsis (⋮) **More actions** button laid out beside it on the icon row, and a single primary tap target per comment row. See *Comment row layout* below.

### Thread action bar

The **Read article** primary button (hidden on self-posts) always takes its own row at the top of the bar — full-width, the main CTA on the thread page. Below it, in a second row, left-to-right: **Upvote** (hidden when logged out) → **Favorite** → **Done** → **More actions ⋮**. Each icon button is 48×48px with ≥8px spacing. The two-row layout applies on every viewport: on mobile it guarantees the primary label never gets squeezed out by the icons, and on desktop it reads cleaner than a single crowded row.

The Upvote button uses HN's triangle shape (solid `▲`), colored `--hn-meta` by default and `--hn-orange` when voted; tapping flips local state optimistically and POSTs `/api/vote` in the background, rolling back and toasting on failure. See item 7 under *Features* for the full round-trip. The **Favorite** button (heart, outline → filled) is the "keepsake" half of the save-this pair; Pin is the "queue" half, and lives in the overflow menu (see below). The **Done** button (Material Symbols `done`, outline → filled on toggle) sits immediately right of Favorite and marks the thread complete: the story is filtered out of every feed (see *Pinned vs. Favorite vs. Done* above) and added to the synced Done list. Done is the most salient per-story action on the comments view — by the time you're reading comments you've usually engaged with the thread, and Done is the common exit. Pinning, by contrast, happens before reading (to save for later) and is more natural from the feed row; on the thread page it's secondary, which is why it moved to the overflow menu. Tapping Done on a pinned story also unpins it; Done and Pin are mutually exclusive. Tapping Done does **not** navigate away; the filled icon is the confirmation, and the user can navigate back when ready. Auto-navigate-on-done is tracked in `TODO.md § Thread action bar` as a possible future change. There is no Done toast — the button state is the single source of truth, matching Favorite.
...
- **Pin / Unpin** — toggles the same pinned state as the row-level pin on feeds. Reachable from the thread page so stories opened directly from a share link (where there's no feed row to tap) can still be pinned. Label flips between "Pin" and "Unpin" based on current state.
