...
- **Downvote is karma-gated by HN.** The `how=down` anchor is only rendered on HN's item page for viewers above a threshold (historically ~500 karma) and not for the viewer's own posts, among other per-item rules. For low-karma viewers the scrape step in `/api/vote` returns 502 (the expected anchor is absent) and the hook toasts. We deliberately don't pre-check this — it would cost an extra item-page fetch per Downvote button render for a minority case — and we also don't try to distinguish karma-gate from a genuine HN error in the toast; the user sees a generic "Could not downvote" message and can treat it as "try another time or another account."
