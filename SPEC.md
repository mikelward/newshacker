...
The sticky orange app header (`<AppHeader>`) carries global chrome only — the hamburger menu, the `newshacker` brand/home link, the offline pill, and the account menu. All feed-scoped actions (Refresh, Undo, Sweep unpinned) live in the separate `FeedActionToolbar` below the header — see *Feed action toolbar (top)* below.

On non-feed pages (thread, `/pinned`, `/done`, `/hidden`, etc.) the app header looks the same: just the hamburger, brand link, optional offline pill, and account menu. The `FeedActionToolbar` below is the only chrome that differs by page.

No hide/sweep toast: the Undo button (in the top `FeedActionToolbar` and the bottom action bar) is the recovery path. Hiding is always deliberate (swipe right, broom, or menu Hide) — scroll-past does not auto-hide. Pin/unpin don't toast either; the pin button's pressed state is the single source of truth for pinned state.

## Feed action toolbar (top)

On feed pages (`/`, `/top`, `/new`, `/best`, etc.) the `FeedActionToolbar` sits directly beneath the orange `<AppHeader>`, on the neutral page background. It's styled after the app header (transparent icon buttons, no borders, 4px gap, 48×48 tap targets) rather than as a card-styled container, so it reads as a second lower-key chrome strip rather than a competing toolbar. It renders three right-aligned icon buttons, in order **Refresh → Undo → Sweep unpinned**, and inherits its width from `.app-main` (720/860px) so it lines up flush with the story rows below.

- **Refresh** (Material Symbols `refresh`, testid `refresh-btn`) — refetches the current feed and pulls cross-device sync state in one go. Same handler the pull-to-refresh gesture already calls (`Promise.all([refetch(), cloudSyncPullNow()])`); wired through `FeedBarContext` so the toolbar doesn't need to know which feed is on screen. The icon spins while the refresh is in flight and the button is disabled (also disabled when offline). Available on every device — pointer and touch alike — so desktop users (for whom pull-to-refresh is unusable) have a real refresh affordance, and touch users get the same button as a faster alternative to the pull gesture.
- **Undo** (Material Symbols `undo`, testid `undo-btn`) — restores the most recent hide action: either the last swipe-to-hide, the last menu "Hide", or the last sweep (the whole batch at once). One level of undo only; recording a new hide replaces the stored batch. Disabled when there is nothing to undo. Not persisted across reloads.
- **Sweep unpinned** (Material Symbols `sweep`, testid `sweep-btn`) — hides every visible unpinned story in one shot. Disabled when there are no unpinned stories to hide. Tapping it plays the **same** slide-right + fade-out as `useSwipeToDismiss` (200ms ease-out, translate by the row's full width so each row leaves the viewport the way a manually swiped row would) on every swept row *together* (one gesture, not a staggered cascade — "sweep" is a single motion), and the actual hide + undo-batch record commits when the animation finishes, so the rows slide in place instead of popping. Readers with `prefers-reduced-motion: reduce` skip both the animation and the delay and see the rows disappear instantly. Double-tap is ignored while a sweep is already playing out.
...
All three buttons stay in place — they're disabled rather than hidden when unavailable — so the layout doesn't jump. The toolbar renders in every StoryList state (normal feed, loading skeleton, error, empty) so the affordances don't disappear when the list is momentarily empty. The toolbar is **feed-only**: library pages (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) and the thread page never render it.

## Bottom action bar (list views)

Every scrolling list view — feed pages (`/top`, `/new`, `/best`, etc.) and library pages (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) — ends in a **bottom action bar** styled to match the top `FeedActionToolbar` (transparent buttons, no borders, 4px gap, 48×48 tap targets on the neutral page background). Both bars read as a matched pair of chrome strips bracketing the story list. The thread page keeps its own card-styled bottom action bar (see *Thread action bar*) — the thread bar still needs a primary-slot CTA ("Read article"/"Back to top"), so its card-styled rows stay put while the list-view bar goes flat.

**Slots, left → right, always in this order:**

1. **Back to top** (left, text + icon). Same `<BackToTopButton>` component as the thread page's bottom-bar Back to top — Material Symbols `vertical_align_top`, calls `window.scrollTo({ top: 0, behavior: 'smooth' })` (browsers short-circuit this to an instant scroll when the user has `prefers-reduced-motion: reduce` set). Always rendered. It sits leftmost so a reader who has learned the thread bar already knows where this button is. Testid `back-to-top` on feeds and library pages alike.
...
3. **Undo** (right cluster, icon only). Mirrors the top `FeedActionToolbar`'s Undo button — same handler, same `canUndo` gate, same copy (`"Undo hide"` / `"Nothing to undo"`). Testid `undo-btn-bottom` (the top toolbar keeps the unsuffixed `undo-btn`). Feed-only — library pages never render this slot.
4. **Hide unpinned** (right cluster, icon only). A second entry point for the sweep action; shares the broom glyph, tooltip (`"Hide unpinned"` / `"Nothing to hide"` when there's nothing to sweep), `aria-label`, and disabled-when-nothing-to-hide state with the top toolbar's sweep button. It's the *same* sweep — clicking it hides the currently fully-visible unpinned rows. Sits immediately to the right of Undo so the two bars (top toolbar + bottom action bar) read as matched pairs of Undo/Sweep icon clusters. Testid `sweep-btn-bottom` (the top toolbar keeps the unsuffixed `sweep-btn`). Feed-only — library pages never render this slot.

Library pages therefore show only the Back-to-top slot; feed pages show all four when a next page is available and the top toolbar's sweep is enabled, or Back-to-top + Undo + Hide-unpinned when the feed's id list has been exhausted, etc. Reaching the end of any long scroll shouldn't require a manual fling to get back up top — Back to top is always there, matching where the thread page puts it.

**Sizing — More is the one that stretches.** On the feed footer (`.story-list__footer--feed`), More is the action a returning reader came for ("give me the next page of stories"), so it's the one that grows to absorb the row's free space. Back to top stays at its natural size — it's a nice-to-have escape hatch, not the reason the reader scrolled to the bottom. The right-hand icon cluster (Undo + Hide unpinned) stays at 48×48 transparent squares, right-pinned as a group via `margin-left: auto` on the leading icon button (Undo); an adjacent-sibling rule (`.list-footer__icon-btn + .list-footer__icon-btn { margin-left: 0 }`) keeps subsequent icons at the default 4px flex gap so the pair sits together rather than spreading. When the feed has exhausted its id list and More is absent, Back to top still stays natural-width and the icon cluster keeps the right-pin on its own. Library footers don't carry the `--feed` modifier, so Back to top grows to fill the row there (it's the only button — a full-width target beats a small one stranded on the left edge).
...
- **Tiered backoff (the knobs these analytics exist to tune).** All values are env-overridable. Most apply to both tracks; a few are comments-track-specific to account for thread growth dynamics.
  - `WARM_REFRESH_CHECK_INTERVAL_SECONDS` (default **30 min**): re-check cadence while the content is "fresh". Both tracks.
  - `WARM_STABLE_CHECK_INTERVAL_SECONDS` (default **2 h**): re-check cadence once the content has been unchanged for ≥ `WARM_STABLE_THRESHOLD_SECONDS`. Both tracks.
  ...
  - `WARM_MAX_STORY_AGE_SECONDS` (default **48 h**): stop re-checking entirely past this. Past this point the user-facing endpoints still serve the cached summary / insights until Upstash itself evicts the record at the 30-day boundary. Both tracks.
  - `WARM_TOP_N` (default **30**): how many feed ids to consider per tick when `?n=` isn't supplied.
  - `WARM_YOUNG_STORY_AGE_SECONDS` (default **2 h**): threshold, measured against HN `story.time` (submission time), for treating a story as "young". *Comments track only.*
  - `WARM_YOUNG_STORY_REFRESH_INTERVAL_SECONDS` (default **10 min**): re-check cadence for comments on young stories — threads grow fast in their first couple of hours, so the default 30-min fresh interval lags behind real comment churn. *Comments track only; articles don't grow after publication.*
