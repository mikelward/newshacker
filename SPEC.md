...
it refreshes stale or missing pinned roots, capped to the newest 30 pins and
...
comments** (only the absent ids, aggregated across all fill stories into a
single capped 30-id batch per sync run — pin-time comment batches are
best-effort and can fail silently, and without the top-up such a thread
stayed comment-less offline until its root went stale; ids past the cap wait
for a later run). The pinned-set-change
...
change event) retries; an HTTP-status failure keeps the 6 h throttle so a
