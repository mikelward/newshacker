...
On feed pages (`/`, `/top`, `/new`, `/best`, etc.) a second bar — the `FeedActionToolbar` — sits directly beneath the orange `<AppHeader>`, visually separate from it (neutral page-background palette, card-coloured rounded buttons). It renders two right-aligned icon buttons, in order **Undo → Sweep unpinned**, and mirrors the bottom action bar at the other end of the feed so the two bars read as a matched pair that brackets the list.
