...
2. **Large, well-spaced hit areas.** Minimum 44×44px per touch tappable (Apple HIG / WCAG 2.5.5 AAA — the project floor), ≥8px dead space between adjacent targets. Story rows sit higher at 48px; the thread action bar **and the comment card's footer icons** drop to 36px under a precise pointer (see *Thread action bar*), by pointer type rather than viewport width. On a comment card that shrink is also what sets the card's vertical rhythm: the footer's height comes from those buttons, not from the 13px meta text sitting inside them, so a 44px band on a mouse-driven screen left the gap under a comment's last line running about 4.4× the gap above its first.
