...
The sticky orange app header (`<AppHeader>`) carries global chrome only — the hamburger menu, the `newshacker` brand/home link, the offline pill, and, on feed pages, a single feed-scoped **Refresh** icon plus the account menu. Sweep and Undo live below the app header in a separate `FeedActionToolbar`, not inside the orange bar itself — see *Feed action toolbar (top)* below.
...
On non-feed pages (thread, `/pinned`, `/done`, `/hidden`, etc.) the feed-scoped Refresh icon does not render; only the hamburger menu, brand link, optional offline pill, and account menu are shown.
...
Both buttons stay in place — they're disabled rather than hidden when unavailable — so the layout doesn't jump. The toolbar renders in every StoryList state (normal feed, loading skeleton, error, empty) so the Undo/Sweep affordances don't disappear when the list is momentarily empty. The toolbar is **feed-only**: library pages (`/pinned`, `/favorites`, `/done`, `/hidden`, `/opened`) and the thread page never render it.
