...
On feed pages the sticky orange header carries two filter switches followed by three feed-scoped action icons on the right, in order **Unread-only → Hot-only → Refresh → Undo → Sweep unpinned**. All five controls stay in place (never shift) so the layout doesn't jump; action icons are disabled when the action is unavailable rather than being hidden, and filter switches are always enabled.

- **Unread-only filter** (slider switch, eye glyph on the right) — when on, hides every story the reader has already opened (either the article or the thread, as tracked by `useOpenedStories`). When off, shows everything. State is persisted in `localStorage` under `newshacker:feedFilters` (a single JSON object with `unreadOnly` and `hotOnly` booleans) and kept in sync across tabs via the `storage` event. `role="switch"` with `aria-checked` reflecting current state.
- **Hot-only filter** (slider switch, flame glyph on the right) — when on, hides every story `isHotStory()` does not flag (i.e. non-trending). When off, shows everything. Same `newshacker:feedFilters` storage blob as the unread-only switch. The two switches are independent — either, neither, or both can be on.
...
Icons are inlined monochrome SVG (Apache 2.0, Google Material Symbols, outlined weight, viewBox `0 -960 960 960`, drawn with `fill="currentColor"`). The eye and flame glyphs used by the filter switches are drawn on a `0 0 24 24` viewBox but follow the same inline, monochrome, `currentColor` convention. No icon font, CSS, or web request is used to load them at runtime.

The filters only apply to the feed's regular story rows. Off-feed pinned rows prepended to the top of the list are always visible — pinning is an explicit, durable "keep this in front of me" action and the filter toggles shouldn't override it. Filters do not affect library pages (`/pinned`, `/done`, `/favorites`, `/hidden`, `/opened`) — they're feed-scoped and the switches only render on feed pages alongside Refresh/Undo/Sweep.
