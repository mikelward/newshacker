...
### Nested-comment offline cache
- **Status: planned.** Today, `prefetchPinnedStory` (above) caches the root + AI summaries + first 30 top-level kids. Nested replies are only warmed when the user expands them online (`prefetchCommentBatch` from `Comment.tsx`). A user who pins a story and goes offline before expanding any subtree gets the top of the conversation but none of the back-and-forth that usually motivates the pin. This section extends the pin/favorite prefetch to walk into the reply tree, bounded by a byte-size cap.
- **No per-comment pinning.** The pin remains a **story-grain** action (rule 9). A per-comment pin would add a third tap zone to the comment row (currently: row body + collapse toggle), conflicting with rule 5's "fewer, larger tap targets", and would force a second storage key alongside `newshacker:pinnedStoryIds`. Story-grain pin + thread-grain cache is the design.
- **Two tiers of cached comments per pinned/favorited story:**
  1. **Locked tier** (unchanged from `### Pin/Favorite offline prefetch` above): root item, AI article summary, AI comments summary, first 30 top-level kids. `gcTime: Infinity` while the story is pinned/favorited; never evicted by the cap below. This is the rule-9 contract.
  2. **Capped tier** (new): nested replies beyond the locked tier, fetched breadth-first at pin/favorite time up to a per-thread byte budget. Evictable across threads when the aggregate cap is exceeded. Eviction only ever drops the capped tier — a still-pinned story is never reduced below its locked tier.
- **Cap by serialized byte size, not count or depth.**
  - **Count** is a poor proxy. One essay-comment easily outweighs fifty one-liners; the actual budget (localStorage quota, ~5 MB per origin) is denominated in bytes.
  - **Depth** would damage the part users most want offline. Long technical reply chains and gripping back-and-forths live deep in HN threads; truncating by nesting level removes the conversation that motivated the pin.
- **Caps:**
  - **Per-thread:** 500 KB of serialized JSON across the capped tier (sum of `JSON.stringify(item).length` for each cached descendant past the locked 30).
  - **Aggregate:** 3 MB across the union of all pinned + favorited threads' capped tiers. Comfortably under the ~5 MB localStorage quota with headroom for the four React Query keys per story plus the `newshacker:*` library keys.
  - **Eviction:** when the aggregate cap is exceeded, drop whole threads' capped tiers least-recently-viewed first (LRU on `lastViewedAt`, written when `Thread.tsx` mounts for the story). The locked tier of each pinned/favorited story is never touched. Unpinning + un-favoriting a story makes its locked tier eligible for normal `gcTime`-based eviction, which is the existing behavior.
- **Truncation order when a single thread exceeds the per-thread cap:**
  - Walk the kids breadth-first from the root. Add each comment to the cache, decrementing the remaining 500 KB budget by `JSON.stringify(item).length`. Stop when the next item would push the thread over budget.
  - Mark the thread record `partial: true` and persist the byte count written.
  - BFS preserves the conversation **shape** (every subtree gets at least its first reply layer before any subtree gets its second) at the cost of dropping the **tail** of long subtrees. A deep technical argument may be cut mid-chain. The UX surfaces this on the first un-cached comment as a non-tappable affordance: "Replies past this point need a connection." The existing `prefetchCommentBatch`-on-expand path fills them in automatically when the user is back online and re-opens the thread.
- **Implementation hooks (sketch, not binding):**
  - Extend `prefetchPinnedStory` and `prefetchFavoriteStory` (and the `prefetchFeedStory` drive-by warm — see below) with a BFS phase after the locked-tier batch.
  - Reuse `prefetchCommentBatch` for each BFS layer's `/api/items?ids=…&fields=full` batch so the existing edge-cache and soft-fail behavior carry over for free.
  - The pinned-cache locking subscriber (`subscribeToPinnedCacheLocking`) already walks the parent chain to lock newly-arrived comment entries; the capped-tier entries pick up the same `gcTime: Infinity` lock without code changes there. Eviction of capped-tier entries is driven by a separate accountant (a new `src/lib/threadCacheBudget.ts` or similar), not by React Query's `gcTime`.
- **Drive-by feed warm stays at the locked tier only.** `prefetchFeedStory` (trending warm for `score > 100` rows in `StoryList`) does **not** get the BFS phase. A user scrolling the feed has not committed to any single story; warming nested kids for every popular row would multiply Firebase pressure by 10-50× per session for storage that the user almost never reads. The pin/favorite action is the explicit user signal that justifies the extra cost.
- **Cost / reliability (rule 11).**
  - **Pin-time cost.** First-pin of a chatty thread rises from 1 `/api/items` batch (~30-60 KB) to as many BFS batches as it takes to reach 500 KB — typically 1-3 additional 32-id batches, capped near ~16 batches at the absolute worst. Each batch is one edge-cached `/api/items` call against our existing proxy. Server-side, a thread that's already warm on the edge costs ~0; a cold thread pays N Firebase reads serially behind the proxy. A user pinning 20 stories in rapid succession downloads up to ~6 MB before the 3 MB aggregate cap kicks in (real numbers are smaller — most threads don't fill 500 KB). Acceptable for an explicit user action.
  - **Steady-state cost.** Zero. The cap doesn't affect ongoing fetches; the existing comment-on-expand path is unchanged when the user is online.
  - **New failure modes.** Bounded. A `/api/items` failure mid-BFS leaves the thread `partial: true` with whatever was written, identical to the existing soft-fail behavior of `prefetchCommentBatch`. localStorage `QuotaExceededError` falls through to the existing React Query persister error path (logged, persistence skipped for that write); the 3 MB aggregate cap exists to keep us well under the ~5 MB browser quota in normal use, so the persister error path is a backstop, not the steady state.
  - **What the user sees if HN is down at pin time.** The existing offline-error UX — root caches if Firebase responds, otherwise the thread is unavailable until next online visit. Nested-cap behavior degrades gracefully to "no nested cached, locked tier only".
- **Open questions to resolve before implementation:**
  - **Byte-size measurement boundary.** Per-item `JSON.stringify(item).length` (cheap, undercounts React Query persister overhead by ~10-15%) vs. measuring at the persister boundary (accurate, requires hooking the persister). *Tentative: per-item with the 500 KB / 3 MB caps already padded to leave headroom.*
  - **Favorite parity.** Default **yes** — favorites are the permanent keepsake (rule 9), so the offline contract should be at least as strong as pin. Same per-action cost, same explicit user signal.
  - **localStorage vs. IndexedDB.** IndexedDB has ~50 MB+ per origin, is async (no main-thread block), and React Query ships an IndexedDB persister adapter. The migration is mechanical and would let the aggregate cap rise to ~10 MB without quota concerns. *Tentative: ship caps on localStorage first; revisit if real-world usage hits the 3 MB aggregate with user complaints. Don't gate this work on the storage swap.*
