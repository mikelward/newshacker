...
- **Sweep unpinned** (Material Symbols `sweep`) — hides every visible unpinned story in one shot. Disabled when there are no unpinned stories to hide. Tapping it plays a short 180ms slide-right + fade-out on every swept row *together* (one gesture, not a staggered cascade — "sweep" is a single motion), and the actual hide + undo-batch record commits when the animation finishes, so the rows fade out in place instead of popping. Readers with `prefers-reduced-motion: reduce` skip both the animation and the delay and see the rows disappear instantly. Double-tap is ignored while a sweep is already playing out.
