...
Operator-only telemetry pipeline behind `/admin`, used to tune the `isHotStory` thresholds in `src/lib/format.ts` (currently `(velocity > 15 points/h ∧ descendants > 10) ∨ (score > 200 ∧ descendants > 100)` — see *Story feeds → /hot*). Captures `{action, id, score, time, descendants, type, isHot, sourceFeed, eventTime, articleOpened, title}` the first time a story is pinned or hidden on each device. Subsequent toggles on the same id don't re-fire — the first action is when the reader formed an opinion, and that's the only event that carries threshold signal.
