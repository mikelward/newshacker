...
On feed pages the sticky header carries two feed-scoped action icons on the right, in order **Undo → Sweep unpinned**. Both icons stay in place (never shift) so the layout doesn't jump; each is disabled when the action is unavailable rather than being hidden. There is no header Refresh button — the pull-to-refresh gesture on touch and the browser reload (Ctrl/Cmd+R, Ctrl+Shift+R for a hard reload) on desktop cover that path, and an idle feed page never visibly drifts inside a session because feed queries refetch on mount and on tab refocus.
