...
- Body shows in full.
- A thin borderless action toolbar appears below the meta row and above the nested replies. Left-to-right it carries **Upvote** (▲), **Downvote** (▼), **Reply on HN** (`news.ycombinator.com/reply?id=:id`, opens in a new tab), and — pushed to the right edge via `margin-left: auto` — the **Collapse** chevron (Material Symbols `expand_circle_up`). The collapse button is the one disclosure control in the expanded state; it mirrors the expand chevron at the card's bottom-right corner when collapsed, so "bottom-right = (un)collapse" holds in both states. Upvote and Downvote share the thread action bar's optimistic-vote path: tapping flips local state immediately, POSTs `/api/vote` in the background, and rolls back + toasts on failure. The voted button paints in `--nh-orange` (shape stays solid — HN's own shape+color convention, matching the story vote button). Switching direction (up → down, or down → up) chains two API hits — `un` then the new direction — because HN models it that way; if the second leg fails after a successful `un` the UI lands at neutral locally rather than restoring the original direction, so the client can't display a vote HN no longer has. Reply is a working link that hands off to HN (newshacker doesn't submit comments itself). Buttons have no individual borders — the strip reads as a single toolbar rather than a row of separate controls — and each is ≥44×44px so the icons stay tappable. Each icon button carries a tooltip + `aria-label` (icon-only buttons need an accessible name; the tooltip is visual-only).
...
The keyboard-accessible disclosure control moves with the chevron. Collapsed state: a real `<button>` wraps the meta row (`aria-expanded="false"`, `aria-label="Expand comment"`). Expanded state: that wrapper becomes plain text and the `<button>` is the trailing toolbar entry (`aria-expanded="true"`, `aria-label="Collapse comment"`, `data-testid="comment-collapse"`). Only one disclosure button exists at a time.
...
- **Comment-card expand/collapse icon (all devices).** The card's bottom-right corner carries a Material Symbols `expand_circle_down` (collapsed) / `expand_circle_up` (expanded) icon — a circled directional chevron, semantically "expand this", not to be confused with plain `expand_more` / `expand_less` chevrons which read as "next". The icon position follows the "bottom-right of the card" rule in both states, but the host element is different: collapsed cards carry the chevron at the right end of the meta row (inside a `.comment__toggle` `<button>` that wraps the whole meta line, with `margin-left: auto` on `.comment__toggle-icon` so the icon lands at the right edge); expanded cards carry it at the right end of the `.comment__toolbar` strip (`.comment__toolbar-button--collapse`, a 44×44 tap target with its own `margin-left: auto` so it's spaced away from the [upvote][downvote][reply] cluster). When expanded the meta row is plain text (`.comment__meta`) — the explicit disclosure control lives in the toolbar, and the row's click handler still collapses on body taps. Visible on every device so the control is obvious regardless of whether the reader taps the card body or aims for the icon.
