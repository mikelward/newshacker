...
| `SUMMARY_MIN_SCORE` | No | Production + Preview | Minimum story score for on-demand + cron summary generation. Default `1` (i.e. the anti-abuse `score > 1` floor). Raise it — e.g. `100` — to throttle generation to only high-score stories during a cost spike. Blank/non-numeric falls back to `1`. Generation only; the feed's own `score > 1` visibility rule is unchanged. Takes effect on the next redeploy. |
| `SUMMARY_GENERATION_DISABLED` | No | Production + Preview | Kill switch. `1`/`true`/`yes`/`on` makes every new summary return `503 { "reason": "generation_disabled" }` before any Gemini/Jina call, and the cron skip its whole tick. **Cached summaries keep serving.** The blunt lever to stop a runaway bill without unsetting `GOOGLE_API_KEY`. Takes effect on the next redeploy. |
