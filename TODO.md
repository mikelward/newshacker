...
- **Offline cache integrity follow-up.** The pinned-story warm now seeds
  thread data from any pinned/library row or newly synced pin and refreshes
  the first comment page, but the broader model is still opportunistic. Audit
  the remaining gaps: the offline indicator should have an explicit
  reachability probe instead of depending solely on the next app fetch to
  fail, and users who care about a full mega-thread need a deliberate
  deeper-cache action rather than the default 30-comment slice.
