...
- **Observability** (in progress, phased — see `OBSERVABILITY.md`):
  ...
  - [ ] Phase 3 — paging wired to the Axiom monitors. Vendor not yet chosen; `OBSERVABILITY.md` Phase 3 carries the selection criteria in full, and the reason no name is written down here. Deliberately not summarized — a partial list here reads as the whole list, and the criteria are derived from that phase's own steps, so they move when it does.
