...
describe('useVote', () => {
  ...
  it('switching from upvote to downvote chains un then down', async () => {
    addVotedId('alice', 50);
    const fetchMock = stubFetch({
      me: 'alice',
      vote: () => new Response(null, { status: 204 }),
    ...
    // Optimistic: locally already swapped.
  ...
  it('direction switch: first leg (un) failing restores the original direction', async () => {
    // Starting state: upvoted. User taps Downvote. The un call fails
    // (server still has the upvote recorded). Local must restore the
    // upvote — server and local agree.
    addVotedId('alice', 70);
    const toasts: ToastOptions[] = [];
    stubFetch({
      me: 'alice',
  ...
  it('direction switch: second leg failing after successful un leaves the comment NEUTRAL (not restored to original)', async () => {
    // Starting state: upvoted. User taps Downvote. `un` succeeds
    // (server now neutral) but `down` fails (e.g. karma gate).
    // Local must end at neutral, not restore upvoted — otherwise we'd
    // tell the viewer they're upvoted while HN has cleared the vote.
    addVotedId('alice', 71);
    const toasts: ToastOptions[] = [];
    let call = 0;
    stubFetch({
      ...
      vote: () => {
        call += 1;
        if (call === 1) return new Response(null, { status: 204 }); // un ok
