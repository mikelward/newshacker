...
// .thread__summary-body / .thread__summary-insights in Thread.css. Used only to
...
// prompt. Tuned so a p75-length summary (~230 chars) reserves 5 lines at a
// common mobile content width (~360px). The estimator scales to fewer
// lines on wider screens automatically.
const ARTICLE_SUMMARY_EXPECTED_CHARS = 230;

// Each insight is capped at ~25 words in the prompt, averaging ~150 chars.
const INSIGHT_EXPECTED_CHARS = 150;
    ...
    >
      ...
      {data ? (
        <div className="thread__summary-footer">
          Generated by Gemini — may be inaccurate.
        </div>
      ) : null}
    </div>
  );
...
function CommentsSummaryCard({ storyId }: { storyId: number }) {
  ...
  return (
    ...
    >
      ...
      {data ? (
        <div className="thread__summary-insights">
          {data.insights.map((insight, i) => (
            <p key={i} className="thread__summary-insight">
              {insight}
            </p>
          ))}
        </div>
