...
// by construction — HN models a single direction per item, so
// upvoting an already-downvoted item un-downvotes first (and vice
// versa). The downvoted set landed with comment downvoting; the
// upvoted set keeps its original `votedStoryIds` storage key so
// existing logged-in readers don't lose their orange arrows on first
// load after the upgrade. The "Story" suffix is now a misnomer (it
// also covers comment ids), but renaming would force a one-off
// localStorage migration for no user-visible benefit, so it stays.
//
...
// device / the HN web UI, that action never lands here — we show the
// arrow as un-voted, and tapping it will 502 on the scrape step
// (HN's item page carries no `how=up` link for an already-voted
// item). That's acceptable; a follow-up can scrape the vote state
// during the item fetch if desired.
//
...
// Clears both directions so a sign-out wipes all voted state for
// the user, not just the upvote half.
...
// per-user and per-direction namespacing.
