...
describe('idbPersistStorage', () => {
  ...
  it('keeps the localStorage blob when the migration write transaction aborts after the put succeeds', async () => {
    // Regression (Codex review on #373): a put request's onsuccess fires
    // before the transaction commits, and quota/disk pressure can abort
    // the transaction afterward. Resolving writes on request success let
    // the migration delete the legacy localStorage blob while the IDB
    // copy never actually landed — losing both. Writes now settle on the
    // transaction's oncomplete/onabort.
    type FakeRequest = { onsuccess?: () => void };
          ...
          const tx: {
            ...
            objectStore: () => ({
              put: () => {
                ...
                setTimeout(() => {
                  request.onsuccess?.();
                  tx.onabort?.(); // the abort lands AFTER request success
                }, 0);
                return request;
              },
            }),
    ...
    vi.stubGlobal('indexedDB', {
      open: () => {
        const request: FakeRequest & { result: typeof fakeDb } = {
          result: fakeDb,
        };
        setTimeout(() => request.onsuccess?.(), 0);
        return request;
    ...
    // First access runs the migration: get misses, put "succeeds" at the
    // request level, then the transaction aborts. The write must reject,
    // so the legacy blob survives for the next boot to migrate.
    expect(await idbPersistStorage.getItem(PERSIST_KEY)).toBeNull();
    expect(window.localStorage.getItem(PERSIST_KEY)).toBe('{"legacy":true}');
  });
